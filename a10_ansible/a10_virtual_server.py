...
def main():
    ...
    ports = module.params['virtual_server_ports']

    #(TODO:HT) Find proper way to pass version arg
    client = Client(host, "3.0", username, password)
    ...
    if state == 'present':
        ...
        for port in ports:
            if 'service_group' in port and port['service_group'] not in checked_service_groups:
                ...
                if port['service_group'] == '':
                    continue
                try:
                    result = client.slb.service_group.get(port['service_group'])
                except Exception:
        ...
        if True:
            ...
            status = axapi_enabled_disabled(slb_virtual_status)
            result = client.slb.virtual_server.create(slb_virtual, slb_virtual_ip)
            ...
            if needs_update(defined_ports, ports) or needs_update(ports, defined_ports):
                result = client.slb.virtual_server.update(slb_virtual, slb_virtual_ip, status, ports)
                if axapi_failure(result):
                    module.fail_json(msg="failed to create the virtual server: %s" % result['response']['err']['msg'])
                changed = True
        ...
        if changed:
            result = client.slb.virtual_server.get(slb_virtual)
        else:
            result = slb_virtual_data
    elif state == 'absent':
        if slb_virtual_exists:
            result = client.slb.virtual_server.delete(slb_virtual)
            changed = True
        else:
    ...
    if changed and write_config:
        write_result = client.system.action.write_memory();
