...
def report_changes(module, result, existing_config, payload):
    if existing_config:
        for k, v in payload["server-ssl-counters"].items():
            if v.lower() == "true":
                v = 1
            elif v.lower() == "false":
                v = 0
            if existing_config["server-ssl-counters"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["server-ssl-counters"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("server-ssl-counters", module)
