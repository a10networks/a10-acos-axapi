...
description:
    - IPv4 Gateway
short_description: Configures A10 vrrp-a.vrid.blade.parameters.tracking.options.gateway.ipv4-gateway
    ...
    if existing_config:
        for k, v in payload["ipv4-gateway"].items():
            if v.lower() == "true":
                ...
                v = 0
            if existing_config["ipv4-gateway"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["ipv4-gateway"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("ipv4-gateway", module)
