...
def get_argspec():
    ...
    rv.update(dict(
        interface_ipv4=dict(type='dict', ipv4_eth=dict(type='str', ), ipv4_mgmt=dict(type='bool', ), ipv4_ve=dict(type='int', )),
        uuid=dict(type='str', ),
        ...
        for k, v in payload["ipv4-addr"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["ipv4-addr"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["ipv4-addr"][k] = v
