...
def get_argspec():
    ...
    rv.update(dict(
        ...
        alternate_server=dict(type='list',alternate=dict(type='int',),alternate_name=dict(type='str',)),
        uuid=dict(type='str',),
        ...
        port_list=dict(type='list',port_number=dict(type='int',required=True,),protocol=dict(type='str',required=True,choices=['tcp','udp']),range=dict(type='int',),template_port=dict(type='str',),template_server_ssl=dict(type='str',),action=dict(type='str',choices=['enable','disable','disable-with-health-check']),no_ssl=dict(type='bool',),health_check=dict(type='str',),health_check_follow_port=dict(type='int',),follow_port_protocol=dict(type='str',choices=['tcp','udp']),health_check_disable=dict(type='bool',),weight=dict(type='int',),conn_limit=dict(type='int',),no_logging=dict(type='bool',),conn_resume=dict(type='int',),stats_data_action=dict(type='str',choices=['stats-data-enable','stats-data-disable']),extended_stats=dict(type='bool',),alternate_port=dict(type='list',alternate=dict(type='int',),alternate_name=dict(type='str',),alternate_server_port=dict(type='int',)),auth_cfg=dict(type='dict',service_principal_name=dict(type='str',)),uuid=dict(type='str',),user_tag=dict(type='str',),sampling_enable=dict(type='list',counters1=dict(type='str',choices=['all','curr_req','total_req','total_req_succ','total_fwd_bytes','total_fwd_pkts','total_rev_bytes','total_rev_pkts','total_conn','last_total_conn','peak_conn','es_resp_200','es_resp_300','es_resp_400','es_resp_500','es_resp_other','es_req_count','es_resp_count','es_resp_invalid_http','total_rev_pkts_inspected','total_rev_pkts_inspected_good_status_code','response_time','fastest_rsp_time','slowest_rsp_time','curr_ssl_conn','total_ssl_conn','resp-count','resp-1xx','resp-2xx','resp-3xx','resp-4xx','resp-5xx','resp-other','resp-latency'])))
    ))
