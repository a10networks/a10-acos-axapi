...
short_description: Configures A10 cgnv6.stateful.firewall.tcp.stun-timeout
...
def get_argspec():
    ...
    rv.update(dict(
        ...
        stun_timeout_val_port_range=dict(type='int',),
        uuid=dict(type='str',),
    ...
    if existing_config:
        for k, v in payload["stun-timeout"].items():
            if v.lower() == "true":
                ...
                v = 0
            if existing_config["stun-timeout"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["stun-timeout"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("stun-timeout", module)
