...
short_description: Configures A10 visibility.monitor.secondary.monitor.debug
...
def new_url(module):
    ...
    url_base = "/axapi/v3/visibility/monitor/secondary-monitor/debug/{debug-ip-addr}+{debug-port}+{debug-protocol}"
        ...
        for k, v in payload["debug"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["debug"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["debug"][k] = v
