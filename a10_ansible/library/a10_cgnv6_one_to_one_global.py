...
def get_argspec():
    ...
    rv.update(dict(
        sampling_enable=dict(type='list', counters1=dict(type='str', choices=['all','total-map-allocated','total-map-freed','map-alloc-failure','map-dbl-free','alloc-map-race','map-not-found','ha-map-mismatch','ha-select-addr-failure','ha-alloc-map-conflicts'])),
        mapping_timeout=dict(type='int', ),
        stats=dict(type='dict', total_map_freed=dict(type='str', ),map_alloc_failure=dict(type='str', ),total_map_allocated=dict(type='str', )),
        uuid=dict(type='str', )
    ))
