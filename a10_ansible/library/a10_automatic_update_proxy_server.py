...
short_description: Configures A10 automatic.update.proxy-server
    ...
    if existing_config:
        for k, v in payload["proxy-server"].items():
            if v.lower() == "true":
                ...
                v = 0
            if existing_config["proxy-server"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["proxy-server"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("proxy-server", module)
