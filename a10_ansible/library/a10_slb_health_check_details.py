...
def report_changes(module, result, existing_config, payload):
    if existing_config:
        for k, v in payload["health-check-details"].items():
            if v.lower() == "true":
                v = 1
            elif v.lower() == "false":
                v = 0
            if existing_config["health-check-details"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["health-check-details"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("health-check-details", module)
