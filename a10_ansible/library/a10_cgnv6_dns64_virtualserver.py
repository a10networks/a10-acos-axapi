...
def get_argspec():
    ...
    rv.update(dict(
        ...
        port_list=dict(type='list',port_number=dict(type='int',required=True,),protocol=dict(type='str',required=True,choices=['dns-udp']),action=dict(type='str',choices=['enable','disable']),pool=dict(type='str',),auto=dict(type='bool',),precedence=dict(type='bool',),service_group=dict(type='str',),template_dns=dict(type='str',),template_policy=dict(type='str',),uuid=dict(type='str',),user_tag=dict(type='str',),sampling_enable=dict(type='list',counters1=dict(type='str',choices=['all','curr_conn','total_l4_conn','total_l7_conn','toatal_tcp_conn','total_conn','total_fwd_bytes','total_fwd_pkts','total_rev_bytes','total_rev_pkts','total_dns_pkts','total_mf_dns_pkts','es_total_failure_actions','compression_bytes_before','compression_bytes_after','compression_hit','compression_miss','compression_miss_no_client','compression_miss_template_exclusion','curr_req','total_req','total_req_succ','peak_conn','curr_conn_rate','last_rsp_time','fastest_rsp_time','slowest_rsp_time'])))
    ))
