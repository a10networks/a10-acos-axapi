...
def get_argspec():
    ...
    rv.update(dict(
        ...
        icmp=dict(type='dict',send_on_port_unavailable=dict(type='str',choices=['host-unreachable','admin-filtered','disable']),send_on_user_quota_exceeded=dict(type='str',choices=['host-unreachable','admin-filtered','disable'])),
        logging=dict(type='dict',default_template=dict(type='str',),pool=dict(type='list',pool_name=dict(type='str',),template=dict(type='str',)),shared=dict(type='bool',),partition_name=dict(type='str',)),
        port_batching=dict(type='dict',size=dict(type='str',choices=['1','8','16','32','64','128','256','512']),tcp_time_wait_interval=dict(type='int',)),
        uuid=dict(type='str',)
    ))
