...
short_description: Configures A10 router.bgp.address.family.ipv6-network.ipv6.network
...
def get_argspec():
    ...
    rv.update(dict(
        ...
        network_ipv6=dict(type='str',required=True,),
    ...
    if existing_config:
        for k, v in payload["ipv6-network"].items():
            if v.lower() == "true":
                ...
                v = 0
            if existing_config["ipv6-network"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["ipv6-network"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("ipv6-network", module)
