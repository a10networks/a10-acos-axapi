...
DOCUMENTATION = """
module: a10_fw_active_rule_set
description:
    - Active firewall policy
short_description: Configures A10 fw.active-rule-set
        ...
        for k, v in payload["active-rule-set"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["active-rule-set"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["active-rule-set"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("active-rule-set", module)
