...
options:
    ...
    sampling_enable:
        ...
        suboptions:
            counters1:
                description:
                - "'all'= all; 'hit-count'= Hit counts; 'permit-bytes'= Permitted bytes counter; 'deny-bytes'= Denied bytes counter; 'reset-bytes'= Reset bytes counter; 'active-session-tcp'= Actvie TCP session counter; 'active-session-udp'= Active UDP session counter; 'active-session-icmp'= Active ICMP session counter; 'active-session-other'= Active other protocol session counter; 'session-tcp'= TCP session counter; 'session-udp'= UDP session counter; 'session-icmp'= ICMP session counter; 'session-other'= Other protocol session counter; "
    service_list:
        description:
        - "Field service_list"
        required: False
...
AVAILABLE_PROPERTIES = ["action","cgnv6_fixed_nat_log","cgnv6_log","cgnv6_lsn_lid","cgnv6_lsn_log","cgnv6_policy","dest_list","dst_ipv4_any","dst_ipv6_any","dst_zone","dst_zone_any","forward_listen_on_port","forward_log","fw_log","fwlog","idle_timeout","ip_version","listen_on_port","log","move_rule","name","oper","policy","remark","sampling_enable","service_any","service_list","source_list","src_ipv4_any","src_ipv6_any","src_zone","src_zone_any","stats","status","user_tag","uuid",]
...
def get_argspec():
    ...
    rv.update(dict(
        oper=dict(type='dict',status=dict(type='str',),denybytes=dict(type='int',),sessionother=dict(type='int',),activesessionother=dict(type='int',),activesessiontcp=dict(type='int',),permitbytes=dict(type='int',),sessiontcp=dict(type='int',),activesessionicmp=dict(type='int',),name=dict(type='str',required=True,),sessionicmp=dict(type='int',),activesessiontotal=dict(type='int',),hitcount=dict(type='int',),sessiontotal=dict(type='int',),totalbytes=dict(type='int',),action=dict(type='str',),resetbytes=dict(type='int',),sessionudp=dict(type='int',),activesessionudp=dict(type='int',)),
