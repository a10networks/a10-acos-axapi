...
short_description: Configures A10 interface.ethernet.ipv6.stateful-firewall
    ...
    if existing_config:
        for k, v in payload["stateful-firewall"].items():
            if v.lower() == "true":
                ...
                v = 0
            if existing_config["stateful-firewall"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["stateful-firewall"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("stateful-firewall", module)
