...
DOCUMENTATION = """
module: a10_counter_port_diameter
description:
    - port diameter counters
short_description: Configures A10 counter.port-diameter
...
def new_url(module):
    ...
    url_base = "/axapi/v3/counter/port-diameter/{sampling-enable}"
...
def report_changes(module, result, existing_config, payload):
    if existing_config:
        for k, v in payload["port-diameter"].items():
            if isinstance(v, str):
                if v.lower() == "true":
                    v = 1
                else:
               ...
               break
                ...
                if existing_config["port-diameter"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["port-diameter"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("port-diameter", module)
