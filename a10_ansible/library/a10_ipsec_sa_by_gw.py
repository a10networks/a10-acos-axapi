...
def report_changes(module, result, existing_config, payload):
    if existing_config:
        for k, v in payload["ipsec_sa_by_gw"].items():
            if v.lower() == "true":
                v = 1
            elif v.lower() == "false":
                v = 0
            if existing_config["ipsec_sa_by_gw"][k] != v:
                if result["changed"] != True:
                    result["changed"] = True
                existing_config["ipsec_sa_by_gw"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("ipsec_sa_by_gw", module)
