...
short_description: Configures A10 file.xml-schema
...
def get_argspec():
    ...
    rv.update(dict(
        oper=dict(type='dict', file_list=dict(type='list', file=dict(type='str', ), template=dict(type='str', ), syntax=dict(type='str', ))),
        ...
        for k, v in payload["xml-schema"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["xml-schema"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["xml-schema"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("xml-schema", module)
