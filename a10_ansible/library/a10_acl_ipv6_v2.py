...
VALID_ACL_LIST_FIELDS = ['seq_num','action','log','protocol','src_ip','src_mask','dst_ip','dst_mask','vlan','name']
...
def main():
    ...
    if acl_remarks and len(acl_remarks) > 0:
        json_post['ipv6_acl']['remark_list'] = acl_remarks

    acl_data = axapi_call(module, session_url + '&method=network.acl.ipv6.search', json.dumps({'name': name}))
    ...
    if state == 'present':
        ...
        if not acl_exists:
            result = axapi_call(module, session_url + '&method=network.acl.ipv6.create', json.dumps(json_post))
            if axapi_failure(result):
                module.fail_json(msg="failed to create the acl: %s" % result['response']['err']['msg'])
        else:
            result = axapi_call(module, session_url + '&method=network.acl.ipv6.update', json.dumps(json_post))
            if axapi_failure(result):
                module.fail_json(msg="failed to create the acl: %s" % result['response']['err']['msg'])
        ...
        if acl_exists:
            result = axapi_call(module, session_url + '&method=network.acl.ipv6.delete', json.dumps({'name': name}))
            changed = True
        else:
