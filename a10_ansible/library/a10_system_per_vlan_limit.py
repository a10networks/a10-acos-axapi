...
short_description: Configures A10 system.per-vlan-limit
        ...
        for k, v in payload["per-vlan-limit"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["per-vlan-limit"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["per-vlan-limit"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("per-vlan-limit", module)
