...
description:
    - Configure Scaleout
short_description: Configures A10 scaleout
...
AVAILABLE_PROPERTIES = ["cluster_devices","cluster_id","device_groups","follow_vcs","local_device","service_config","user_tag","uuid",]
...
def get_argspec():
    ...
    rv.update(dict(
        local_device=dict(type='dict',priority=dict(type='int',),id=dict(type='int',),uuid=dict(type='str',)),
...
def new_url(module):
    ...
    url_base = "/axapi/v3/scaleout/{cluster-id}"
        ...
        for k, v in payload["scaleout"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["scaleout"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["scaleout"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("scaleout", module)
