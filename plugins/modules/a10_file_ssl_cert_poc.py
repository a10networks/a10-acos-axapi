...
AVAILABLE_PROPERTIES = ["action", "certificate_type", "dst_file", "file", "file_handle", "pfx_password", ]
...
def create(module, result, payload={}):
    ...
    result["modified_values"].update(**call_result["request_body"]["ssl-cert-poc"])
    result["changed"] = True
    return result
...
def present(module, result, existing_config):
    payload = utils.build_json("ssl-cert-poc", module.params, AVAILABLE_PROPERTIES)
...
def run_command(module):
    ...
    try:
        ...
        existing_config, file_exists = api_client.get_file(module.client, "ssl-cert-poc", file_url, module.params['file'])
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["ssl-cert-poc"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ssl-cert-poc-list"] if info != "NotFound" else info
    except a10_ex.ACOSException as ex:
