...
DOCUMENTATION = r'''
module: a10_ddos_notification_template
description:
...
AVAILABLE_PROPERTIES = ["api", "debug_mode", "disable", "name", "test_connectivity", "user_tag", "uuid", "verbose", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/notification-template/{name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/notification-template"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["notification-template"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["notification-template"].get(k) != v:
            change_results["changed"] = True
            config_changes["notification-template"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("notification-template", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["notification-template"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["notification-template-list"] if info != "NotFound" else info
