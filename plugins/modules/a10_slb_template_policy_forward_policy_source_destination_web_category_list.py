...
short_description: Configures A10 slb.template.policy.forward.policy.source.destination.web-category-list
...
options:
    ...
    name:
        description:
        - Key to identify parent object    policy_name:
        description:
    ...
    if existing_config:
        for k, v in payload["web-category-list"].items():
            if isinstance(v, str):
                ...
                if existing_config["web-category-list"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["web-category-list"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("web-category-list", module)
