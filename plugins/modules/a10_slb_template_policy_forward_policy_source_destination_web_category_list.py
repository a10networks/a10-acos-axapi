...
DOCUMENTATION = r'''
module: a10_slb_template_policy_forward_policy_source_destination_web_category_list
description:
...
def get_argspec():
    ...
    rv.update({'web_category_list': {'type': 'str', 'required': True, },
        'action': {'type': 'str', },
        'ntype': {'type': 'str', 'choices': ['host', 'url']},
        'priority': {'type': 'int', },
        'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'hits']}},
        'stats': {'type': 'dict', 'hits': {'type': 'str', }, 'web_category_list': {'type': 'str', 'required': True, }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    f_dict = {}
    if '/' in str(module.params["web_category_list"]):
        f_dict["web_category_list"] = module.params["web_category_list"].replace("/","%2F")
    else:
        f_dict["web_category_list"] = module.params["web_category_list"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["web-category-list"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["web-category-list"].get(k) != v:
            change_results["changed"] = True
            config_changes["web-category-list"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("web-category-list", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["web-category-list-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["web-category-list"]["stats"] if info != "NotFound" else info
