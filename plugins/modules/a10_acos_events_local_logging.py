    ...
    rv.update({
        ...
        'sampling_enable': {
            ...
            'counters1': {
                ...
                'str',
                'choices': [
                    'all', 'init-pass', 'init-fail', 'freed',
                    'disk-over-thres', 'rate-limited', 'not-inited',
                    'sent-to-store', 'sent-to-store-fail', 'store-fail',
                    'in-logs', 'in-bytes', 'in-logs-backlog',
                    'in-bytes-backlog', 'in-store-fail-no-space',
                    'in-discard-logs', 'in-discard-bytes', 'out-logs',
                    'out-bytes', 'out-error', 'remaining-logs',
                    'remaining-bytes', 'moved-to-delq', 'out-retry',
                    'out-retry-fail', 'curr-total-chunks', 'curr-mem-chunks',
                    'curr-fs-chunks', 'curr-fs-chunks-up',
                    'curr-fs-chunks-down', 'in-logs-agg', 'in-bytes-agg',
                    'in-logs-backlog-agg', 'in-bytes-backlog-agg',
                    'in-store-fail-no-space-agg', 'in-discard-logs-agg',
                    'in-discard-bytes-agg', 'out-logs-agg', 'out-bytes-agg',
                    'out-error-agg', 'out-retry-agg', 'out-retry-fail-agg',
                    'in-logs-curr-agg', 'in-bytes-curr-agg',
                    'in-logs-backlog-curr-agg', 'in-bytes-backlog-curr-agg',
                    'in-discard-logs-curr-agg', 'in-discard-bytes-curr-agg',
                    'out-logs-curr-agg', 'out-bytes-curr-agg'
                ]
            }
        },
        'stats': {
...
def present(module, result, existing_config):
    payload = utils.build_json("local-logging", module.params,
                               AVAILABLE_PROPERTIES)
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info[
                    "local-logging"] if info != "NotFound" else info
                ...
                result["acos_info"] = info[
                    "local-logging-list"] if info != "NotFound" else info
                ...
                result["acos_info"] = info["local-logging"][
