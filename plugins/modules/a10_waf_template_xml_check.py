...
def get_argspec():
    ...
    rv.update({'disable': {'type': 'bool', },
        ...
        'max_entity_decl': {'type': 'int', },
        'max_entity_depth': {'type': 'int', },
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/waf/template/{template_name}/xml-check"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["xml-check"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["xml-check"].get(k) != v:
            change_results["changed"] = True
            config_changes["xml-check"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("xml-check", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["xml-check"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["xml-check-list"] if info != "NotFound" else info
