...
DOCUMENTATION = r'''
module: a10_slb_template_dns_recursive_dns_resolution_gateway_health_check
description:
...
AVAILABLE_PROPERTIES = ["gwhc_ns_cache_lookup", "interval", "num_query_type", "query_name", "retry", "retry_multi", "str_query_type", "timeout", "up_retry", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/slb/template/dns/{dns_name}/recursive-dns-resolution/gateway-health-check"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["gateway-health-check"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["gateway-health-check"].get(k) != v:
            change_results["changed"] = True
            config_changes["gateway-health-check"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("gateway-health-check", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["gateway-health-check"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["gateway-health-check-list"] if info != "NotFound" else info
