...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["destination-ip-address"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["destination-ip-address"].get(k) != v:
            change_results["changed"] = True
            config_changes["destination-ip-address"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("destination-ip-address", module.params, AVAILABLE_PROPERTIES)
