...
DOCUMENTATION = r'''
module: a10_ddos_network_object_sub_network_sub_network_v6
description:
...
AVAILABLE_PROPERTIES = ["host_anomaly_threshold", "sampling_enable", "stats", "sub_network_anomaly_threshold", "subnet_breakdown", "subnet_ipv6_addr", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object/{network_object_object_name}/sub-network/sub-network-v6/{subnet_ipv6_addr}"

    f_dict = {}
    if '/' in str(module.params["subnet_ipv6_addr"]):
        f_dict["subnet_ipv6_addr"] = module.params["subnet_ipv6_addr"].replace("/", "%2F")
    else:
        f_dict["subnet_ipv6_addr"] = module.params["subnet_ipv6_addr"]
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object/{network_object_object_name}/sub-network/sub-network-v6"

    f_dict = {}
    f_dict["subnet_ipv6_addr"] = ""
    f_dict["network_object_object_name"] = module.params["network_object_object_name"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["sub-network-v6"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["sub-network-v6"].get(k) != v:
            change_results["changed"] = True
            config_changes["sub-network-v6"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("sub-network-v6", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["sub-network-v6"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["sub-network-v6-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["sub-network-v6"]["stats"] if info != "NotFound" else info
