...
def get_argspec():
    ...
    rv.update({'complexity': {'type': 'str', 'choices': ['Strict', 'Medium', 'Simple']},
        'aging': {'type': 'str', 'choices': ['Strict', 'Medium', 'Simple']},
        'history': {'type': 'str', 'choices': ['Strict', 'Medium', 'Simple']},
        'min_pswd_len': {'type': 'int', },
        'uuid': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["password-policy"].items():
        ...
        if config_changes["password-policy"].get(k) != v:
            change_results["changed"] = True
            config_changes["password-policy"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("password-policy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["password-policy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["password-policy-list"] if info != "NotFound" else info
