...
DOCUMENTATION = r'''
module: a10_ddos_dns_cache_config
description:
    - DNS Cache Settings
author: A10 Networks
options:
    ...
    max_concurrent_zone_transfers:
        description:
        - "Field max_concurrent_zone_transfers"
        type: dict
        required: False
        suboptions:
            warm_up_mode:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dns-cache-config"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-cache-config"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["dns-cache-config"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-cache-config"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-cache-config", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dns-cache-config"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dns-cache-config-list"] if info != "NotFound" else info
