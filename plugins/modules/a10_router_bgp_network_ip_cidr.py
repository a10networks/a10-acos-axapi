...
def existing_url(module):
    ...
    url_base = "/axapi/v3/router/bgp/{bgp_as_number}/network/ip-cidr/{network_ipv4_cidr}"

    f_dict = {}
    if '/' in str(module.params["network_ipv4_cidr"]):
        f_dict["network_ipv4_cidr"] = module.params["network_ipv4_cidr"].replace("/", "%2F")
    else:
        f_dict["network_ipv4_cidr"] = module.params["network_ipv4_cidr"]
...
def new_url(module):
    ...
    url_base = "/axapi/v3/router/bgp/{bgp_as_number}/network/ip-cidr"

    f_dict = {}
    f_dict["network_ipv4_cidr"] = ""
    f_dict["bgp_as_number"] = module.params["bgp_as_number"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ip-cidr"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["ip-cidr"].get(k) != v:
            change_results["changed"] = True
            config_changes["ip-cidr"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ip-cidr", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["ip-cidr"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ip-cidr-list"] if info != "NotFound" else info
