...
options:
    ...
    max_packet_queue_time:
        description:
        - "Configure netflow packet queue time (Max packet queue time(*20ms). Default=50(
          *20ms = 1s)))"
        type: int
        required: False
    reset_time_on_flow_record:
        description:
        - "Reset session start time to current time on each flow timeout export for long-
...
def get_argspec():
    ...
    rv.update({'max_packet_queue_time': {'type': 'int', },
        'reset_time_on_flow_record': {'type': 'bool', },
        'nat44_tpl_1001': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    return rv
