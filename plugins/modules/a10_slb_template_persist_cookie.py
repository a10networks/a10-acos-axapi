...
AVAILABLE_PROPERTIES = ["cookie_name", "domain", "dont_honor_conn_rules", "encrypt_level", "encrypted", "expire", "httponly", "insert_always", "match_type", "name", "pass_phrase", "pass_thru", "path", "prefix", "samesite", "scan_all_members", "secure", "server", "server_service_group", "service_group", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        'dont_honor_conn_rules': {'type': 'bool', },
        'expire': {'type': 'int', },
        'insert_always': {'type': 'bool', },
        'encrypt_level': {'type': 'int', },
        'pass_phrase': {'type': 'str', },
        'encrypted': {'type': 'str', },
        'cookie_name': {'type': 'str', },
        'prefix': {'type': 'str', 'choices': ['host', 'secure']},
        'domain': {'type': 'str', },
        'samesite': {'type': 'str', 'choices': ['none', 'lax', 'strict']},
        'path': {'type': 'str', },
        'pass_thru': {'type': 'bool', },
        'secure': {'type': 'bool', },
        'httponly': {'type': 'bool', },
        'match_type': {'type': 'bool', },
        'server': {'type': 'bool', },
        'server_service_group': {'type': 'bool', },
        'scan_all_members': {'type': 'bool', },
        'service_group': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["cookie"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["cookie"].get(k) != v:
            change_results["changed"] = True
            config_changes["cookie"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("cookie", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["cookie"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["cookie-list"] if info != "NotFound" else info
