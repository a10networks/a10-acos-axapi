...
def get_argspec():
    ...
    rv.update({'external': {'type': 'bool', },
        'ext_program': {'type': 'str', },
        'shared_partition_program': {'type': 'bool', },
        'ext_program_shared': {'type': 'str', },
        'ext_port': {'type': 'int', },
        'ext_arguments': {'type': 'str', },
        'ext_preference': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["external"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["external"].get(k) != v:
            change_results["changed"] = True
            config_changes["external"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("external", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["external"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["external-list"] if info != "NotFound" else info
