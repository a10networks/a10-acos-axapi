...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates_template_trigger_sys_obj_stats_change_cgnv6_lsn
description:
    - Configure triggers for cgnv6.lsn.global object
    ...
    trigger_stats_inc:
        ...
        suboptions:
            ...
            sip_alg_alloc_single_port_failure:
                description:
                - "Enable automatic packet-capture for SIP ALG Alloc Single RTP or RTCP NAT Port
          Failure"
                type: bool
            sip_alg_create_single_fullcone_failure:
                description:
                - "Enable automatic packet-capture for SIP ALG Create Single RTP or RTCP Full-cone
          Session Failure"
                type: bool
            sip_alg_create_rtp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for SIP ALG Create RTP Full-cone Session
          Failure"
                type: bool
            sip_alg_create_rtcp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for SIP ALG Create RTCP Full-cone Session
          Failure"
                type: bool
            h323_alg_alloc_single_port_failure:
                description:
                - "Enable automatic packet-capture for H323 ALG Alloc Single RTP or RTCP NAT Port
          Failure"
                type: bool
            h323_alg_create_single_fullcone_failure:
                description:
                - "Enable automatic packet-capture for H323 ALG Create Single RTP or RTCP Full-
          cone Session Failure"
                type: bool
            h323_alg_create_rtp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for H323 ALG Create RTP Full-cone Session
          Failure"
                type: bool
            h323_alg_create_rtcp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for H323 ALG Create RTCP Full-cone Session
          Failure"
                type: bool
            ...
            fullcone_ext_mem_alloc_failure:
                description:
                - "Enable automatic packet-capture for LSN Fullcone Extension Memory Allocate
          Failure"
                type: bool
            fullcone_ext_mem_alloc_init_faulure:
                description:
                - "Enable automatic packet-capture for LSN Fullcone Extension Initialization
          Failure"
                type: bool
            mgcp_alg_create_rtp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for MGCP ALG Create RTP Full-cone Session
          Failure"
                type: bool
            mgcp_alg_create_rtcp_fullcone_failure:
                description:
                - "Enable automatic packet-capture for MGCP ALG Create RTCP Full-cone Session
          Failure"
                type: bool
            mgcp_alg_port_pair_alloc_from_quota_par:
                description:
                - "Enable automatic packet-capture for MGCP ALG Port Pair Allocated From Quota
          Partition Error"
                type: bool
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/cgnv6-lsn"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["cgnv6-lsn"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["cgnv6-lsn"].get(k) != v:
            change_results["changed"] = True
            config_changes["cgnv6-lsn"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("cgnv6-lsn", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["cgnv6-lsn"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["cgnv6-lsn-list"] if info != "NotFound" else info
