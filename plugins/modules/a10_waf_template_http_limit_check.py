...
def existing_url(module):
    ...
    url_base = "/axapi/v3/waf/template/{template_name}/http-limit-check"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["http-limit-check"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["http-limit-check"].get(k) != v:
            change_results["changed"] = True
            config_changes["http-limit-check"][k] = v
