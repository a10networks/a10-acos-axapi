...
DOCUMENTATION = r'''
module: a10_ddos_dst_zone_detection_outbound_detection
description:
...
options:
    ...
    enable_top_k:
        ...
        suboptions:
            ...
            topk_netmask:
                description:
                - "Subnet mask. The value should be less than or equal to the minimum zone subnet
          mask + 8 (IPv6 Subnet mask)"
                type: int
            topk_num_records:
                description:
    ...
    indicator_list:
        ...
        suboptions:
          ...
          'syn-rate'= rate on incoming SYN packets; 'fin-rate'= rate on incoming FIN
          packets; 'rst-rate'= rate of incoming RST packets; 'small-window-ack-rate'=
          rate of small window advertisement; 'empty-ack-rate'= rate of incoming packets
          which have no payload; 'small-payload-rate'= rate of short payload packet;
          'syn-fin-ratio'= ratio of incoming SYN packet rate divided by the rate of
...
AVAILABLE_PROPERTIES = ["configuration", "discovery_method", "discovery_record", "enable_top_k", "indicator_list", "oper", "toggle", "topk_source_subnet", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/zone/{zone_name}/detection/outbound-detection"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["outbound-detection"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["outbound-detection"].get(k) != v:
            change_results["changed"] = True
            config_changes["outbound-detection"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("outbound-detection", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["outbound-detection"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["outbound-detection-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["outbound-detection"]["oper"] if info != "NotFound" else info
