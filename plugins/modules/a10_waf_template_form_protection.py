...
AVAILABLE_PROPERTIES = ["csrf_check", "enable_disable_action", "field_consistency_check", "form_check_caching", "form_check_non_post", "form_check_non_ssl", "form_check_request_non_post", "form_check_response_non_post", "form_check_response_non_post_sanitize", "password_check_autocomplete", "password_check_non_masked", "password_check_non_ssl", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'enable_disable_action': {'type': 'str', 'choices': ['enable', 'disable']},
        ...
        'field_consistency_check': {'type': 'bool', },
        'password_check_non_masked': {'type': 'bool', },
        'password_check_non_ssl': {'type': 'bool', },
        'password_check_autocomplete': {'type': 'bool', },
        'form_check_non_ssl': {'type': 'bool', },
        'form_check_caching': {'type': 'bool', },
        'form_check_non_post': {'type': 'bool', },
        'form_check_request_non_post': {'type': 'bool', },
        'form_check_response_non_post': {'type': 'bool', },
        'form_check_response_non_post_sanitize': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/waf/template/{template_name}/form-protection"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["form-protection"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["form-protection"].get(k) != v:
            change_results["changed"] = True
            config_changes["form-protection"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("form-protection", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["form-protection"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["form-protection-list"] if info != "NotFound" else info
