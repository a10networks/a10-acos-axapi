...
AVAILABLE_PROPERTIES = ["action", "health_check", "health_check_disable", "port_number", "protocol", "sampling_enable", "stats", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'port_number': {'type': 'int', 'required': True, },
        ...
        'stats': {'type': 'dict', 'msgs_sent': {'type': 'str', }, 'port_number': {'type': 'int', 'required': True, }, 'protocol': {'type': 'str', 'required': True, 'choices': ['tcp', 'udp']}}
    })
    # Parent keys
    rv.update(dict(
        log_server_name=dict(type='str', required=True),
    ))
    return rv
...
def existing_url(module):
    ...
    if '/' in module.params["log_server_name"]:
        f_dict["log_server_name"] = module.params["log_server_name"].replace("/","%2F")
    else:
