...
def get_argspec():
    ...
    rv.update({'file_path': {'type': 'str', },'file': {'type': 'str', },
        'action': {'type': 'str', 'choices': ['export']},
        'file_handle': {'type': 'str', },
        'dst_file': {'type': 'str', },
        'uuid': {'type': 'str', },
        'oper': {'type': 'dict', 'ssl_csr': {'type': 'list', 'name': {'type': 'str', }, 'ntype': {'type': 'str', }, 'common_name': {'type': 'str', }, 'organization': {'type': 'str', }, 'subject': {'type': 'str', }, 'status': {'type': 'str', }}, 'sortby_name': {'type': 'bool', }}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["csr"].items():
        ...
        if config_changes["csr"].get(k) != v:
            change_results["changed"] = True
            config_changes["csr"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("csr", module.params, AVAILABLE_PROPERTIES)
...
def _cert_delete(module):
    params = {"delete": {}}
    params['delete'] = {"csr": "{}".format(module.params['file'])}
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["csr"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["csr-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module),
                                                      params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["csr"]["oper"] if info != "NotFound" else info
    except a10_ex.ACOSException as ex:
