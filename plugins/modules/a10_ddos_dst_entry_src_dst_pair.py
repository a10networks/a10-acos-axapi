...
DOCUMENTATION = r'''
module: a10_ddos_dst_entry_src_dst_pair
description:
    - per-Source-Destination pair Source-limiting configuration
author: A10 Networks
options:
    state:
        description:
...
AVAILABLE_PROPERTIES = ["app_type_src_dst_list", "bypass", "default", "exceed_log_cfg", "glid", "l4_type_src_dst_list", "log_periodic", "template", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/src-dst-pair"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["src-dst-pair"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["src-dst-pair"].get(k) != v:
            change_results["changed"] = True
            config_changes["src-dst-pair"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("src-dst-pair", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["src-dst-pair"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["src-dst-pair-list"] if info != "NotFound" else info
