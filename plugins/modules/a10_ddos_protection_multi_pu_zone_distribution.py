...
DOCUMENTATION = r'''
module: a10_ddos_protection_multi_pu_zone_distribution
description:
    - Multi pu traffic distribution mode configuration
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/protection/multi-pu-zone-distribution"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["multi-pu-zone-distribution"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["multi-pu-zone-distribution"].get(k) != v:
            change_results["changed"] = True
            config_changes["multi-pu-zone-distribution"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("multi-pu-zone-distribution", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["multi-pu-zone-distribution"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["multi-pu-zone-distribution-list"] if info != "NotFound" else info
