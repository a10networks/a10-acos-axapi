...
AVAILABLE_PROPERTIES = ["bot_check", "bot_check_policy_file", "command_injection_check", "command_injection_check_policy_file", "lifetime", "redirect_whitelist", "referer_check", "referer_domain_list", "referer_domain_list_only", "referer_safe_url", "session_check", "sqlia_check", "sqlia_check_policy_file", "url_blacklist", "url_learned_list", "url_whitelist", "uuid", "waf_blacklist_file", "waf_whitelist_file", "xss_check", "xss_check_policy_file", ]
...
def get_argspec():
    ...
    rv.update({'bot_check': {'type': 'bool', },
        'bot_check_policy_file': {'type': 'str', },
        'command_injection_check': {'type': 'str', 'choices': ['cookies', 'headers', 'form-body', 'uri-query']},
        'command_injection_check_policy_file': {'type': 'str', },
        'redirect_whitelist': {'type': 'bool', },
        ...
        'lifetime': {'type': 'int', },
        'sqlia_check': {'type': 'str', 'choices': ['reject']},
        'sqlia_check_policy_file': {'type': 'str', },
        'url_blacklist': {'type': 'bool', },
        'waf_blacklist_file': {'type': 'str', },
        'url_whitelist': {'type': 'bool', },
        'waf_whitelist_file': {'type': 'str', },
        'url_learned_list': {'type': 'bool', },
        'xss_check': {'type': 'str', 'choices': ['reject']},
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/waf/template/{template_name}/request-check"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["request-check"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["request-check"].get(k) != v:
            change_results["changed"] = True
            config_changes["request-check"][k] = v
