...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_object_templates_interface_tunnel_tmpl
description:
    - Configure template for interface.tunnel
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'trigger_stats_inc': {'type': 'dict', 'num_rx_err_pkts': {'type': 'bool', }, 'num_tx_err_pkts': {'type': 'bool', }, 'uuid': {'type': 'str', }},
        'trigger_stats_rate': {'type': 'dict', 'threshold_exceeded_by': {'type': 'int', }, 'duration': {'type': 'int', }, 'num_rx_err_pkts': {'type': 'bool', }, 'num_tx_err_pkts': {'type': 'bool', }, 'uuid': {'type': 'str', }}
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/object-templates/interface-tunnel-tmpl/{name}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["interface-tunnel-tmpl"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["interface-tunnel-tmpl"].get(k) != v:
            change_results["changed"] = True
            config_changes["interface-tunnel-tmpl"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("interface-tunnel-tmpl", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["interface-tunnel-tmpl"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["interface-tunnel-tmpl-list"] if info != "NotFound" else info
