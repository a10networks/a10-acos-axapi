    ...
    trigger_stats_inc:
        ...
        suboptions:
            ...
            dnsrrl_total_dropped:
                description:
                - "Enable automatic packet-capture for DNS Response-Rate-Limiting Total Responses
          Dropped"
                type: bool
            dnsrrl_bad_fqdn:
                description:
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'trigger_stats_inc': {'type': 'dict', 'total_mf_dns_pkts': {'type': 'bool', }, 'es_total_failure_actions': {'type': 'bool', }, 'compression_miss_no_client': {'type': 'bool', }, 'compression_miss_template_exclusion': {'type': 'bool', }, 'loc_deny': {'type': 'bool', }, 'dnsrrl_total_dropped': {'type': 'bool', }, 'dnsrrl_bad_fqdn': {'type': 'bool', }, 'uuid': {'type': 'str', }},
        'trigger_stats_rate': {'type': 'dict', 'threshold_exceeded_by': {'type': 'int', }, 'duration': {'type': 'int', }, 'total_mf_dns_pkts': {'type': 'bool', }, 'es_total_failure_actions': {'type': 'bool', }, 'compression_miss_no_client': {'type': 'bool', }, 'compression_miss_template_exclusion': {'type': 'bool', }, 'loc_deny': {'type': 'bool', }, 'dnsrrl_total_dropped': {'type': 'bool', }, 'dnsrrl_bad_fqdn': {'type': 'bool', }, 'uuid': {'type': 'str', }}
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/object-templates/slb-vport-tmpl/{name}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["slb-vport-tmpl"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["slb-vport-tmpl"].get(k) != v:
            change_results["changed"] = True
            config_changes["slb-vport-tmpl"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("slb-vport-tmpl", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["slb-vport-tmpl"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["slb-vport-tmpl-list"] if info != "NotFound" else info
