...
AVAILABLE_PROPERTIES = ["debug_list", "delete_debug_file", "mon_entity_topk", "replay_debug_file", "secondary_monitoring_key", "source_entity_topk", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'secondary_monitoring_key': {'type': 'str', 'required': True, 'choices': ['service']},
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/monitor/secondary-monitor"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["secondary-monitor"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["secondary-monitor"].get(k) != v:
            change_results["changed"] = True
            config_changes["secondary-monitor"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("secondary-monitor", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["secondary-monitor"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["secondary-monitor-list"] if info != "NotFound" else info
