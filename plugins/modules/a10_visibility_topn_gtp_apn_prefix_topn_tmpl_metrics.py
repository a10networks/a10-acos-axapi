...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(
        gtp_apn_prefix_topn_tmpl_name=dict(type='str', required=True),
    ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/topn/gtp-apn-prefix-topn-tmpl/{gtp_apn_prefix_topn_tmpl_name}/metrics"

    f_dict = {}
    if '/' in module.params["gtp_apn_prefix_topn_tmpl_name"]:
        f_dict["gtp_apn_prefix_topn_tmpl_name"] = module.params["gtp_apn_prefix_topn_tmpl_name"].replace("/","%2F")
    else:
        f_dict["gtp_apn_prefix_topn_tmpl_name"] = module.params["gtp_apn_prefix_topn_tmpl_name"]
