...
DOCUMENTATION = r'''
module: a10_template_gtp_logging_policy
description:
    ...
    log:
        ...
        suboptions:
            ...
            sequence_num_correlation:
                description:
                - "Logging Packet Drop due to mismatch in Sequence number between GTP request and
          response"
                type: bool
...
AVAILABLE_PROPERTIES = ["log", "name", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        'log': {'type': 'dict', 'mandatory_ie_check': {'type': 'bool', }, 'out_of_state_ie_check': {'type': 'bool', }, 'out_of_order_ie_check': {'type': 'bool', }, 'invalid_teid_check': {'type': 'bool', }, 'reserved_ie_check': {'type': 'bool', }, 'message_filtering': {'type': 'bool', }, 'apn_imsi_filtering': {'type': 'bool', }, 'rat_type_filtering': {'type': 'bool', }, 'msisdn_filtering': {'type': 'bool', }, 'crosslayer_correlation': {'type': 'bool', }, 'anti_spoofing_check': {'type': 'bool', }, 'msisdn_imsi_correlation': {'type': 'bool', }, 'max_message_length_check': {'type': 'bool', }, 'gtp_in_gtp_filtering': {'type': 'bool', }, 'sequence_num_correlation': {'type': 'bool', }, 'invalid_header_check': {'type': 'bool', }},
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', }
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/template/gtp/logging-policy/{name}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["logging-policy"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["logging-policy"].get(k) != v:
            change_results["changed"] = True
            config_changes["logging-policy"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("logging-policy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["logging-policy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["logging-policy-list"] if info != "NotFound" else info
