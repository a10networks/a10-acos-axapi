...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-naptr-record/{naptr_target}+{service_proto}+{flag}"

    f_dict = {}
    if '/' in str(module.params["naptr_target"]):
        f_dict["naptr_target"] = module.params["naptr_target"].replace(
            "/", "%2F")
    else:
        f_dict["naptr_target"] = module.params["naptr_target"]
    if '/' in str(module.params["service_proto"]):
        f_dict["service_proto"] = module.params["service_proto"].replace(
            "/", "%2F")
    else:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-naptr-record"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["dns-naptr-record"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-naptr-record"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-naptr-record", module.params,
                               AVAILABLE_PROPERTIES)
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info[
                    "dns-naptr-record"] if info != "NotFound" else info
                ...
                result["acos_info"] = info[
                    "dns-naptr-record-list"] if info != "NotFound" else info
                ...
                result["acos_info"] = info["dns-naptr-record"][
