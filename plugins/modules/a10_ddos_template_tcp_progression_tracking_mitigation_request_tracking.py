...
DOCUMENTATION = r'''
module: a10_ddos_template_tcp_progression_tracking_mitigation_request_tracking
description:
...
AVAILABLE_PROPERTIES = [
    "first_request_max_time", "progression_tracking_req_action", "progression_tracking_req_action_list_name", "progression_tracking_req_enabled", "request_length_max", "request_length_min", "request_response_model", "request_to_response_max_time", "response_length_max", "response_length_min", "response_to_request_max_time", "uuid", "violation",
    ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/template/tcp/{tcp_name}/progression-tracking/mitigation/request-tracking"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["request-tracking"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["request-tracking"].get(k) != v:
            change_results["changed"] = True
            config_changes["request-tracking"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("request-tracking", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["request-tracking"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["request-tracking-list"] if info != "NotFound" else info
