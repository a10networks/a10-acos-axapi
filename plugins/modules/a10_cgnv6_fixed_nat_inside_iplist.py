...
AVAILABLE_PROPERTIES = ["dest_rule_list", "dynamic_pool_size", "inside_ip_list", "method", "nat_end_address", "nat_ip_list", "nat_netmask", "nat_start_address", "offset", "partition", "ports_per_user", "respond_to_user_mac", "service_config_template", "session_quota", "skip_ports_on_rollover", "usable_nat_ports", "uuid", "vrid", ]
...
def existing_url(module):
    ...
    f_dict = {}
    if '/' in str(module.params["inside_ip_list"]):
        f_dict["inside_ip_list"] = module.params["inside_ip_list"].replace("/", "%2F")
    else:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["iplist"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["iplist"].get(k) != v:
            change_results["changed"] = True
            config_changes["iplist"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("iplist", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["iplist"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["iplist-list"] if info != "NotFound" else info
