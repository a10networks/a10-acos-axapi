...
DOCUMENTATION = r'''
module: a10_interface_ethernet_ip_ospf_ospf_ip
description:
...
def get_argspec():
    ...
    rv.update({'ip_addr': {'type': 'str', 'required': True, },
        'authentication': {'type': 'bool', },
        'value': {'type': 'str', 'choices': ['message-digest', 'null']},
        'authentication_key': {'type': 'str', },
        'cost': {'type': 'int', },
        'database_filter': {'type': 'str', 'choices': ['all']},
        'out': {'type': 'bool', },
        'dead_interval': {'type': 'int', },
        'hello_interval': {'type': 'int', },
        'message_digest_cfg': {'type': 'list', 'message_digest_key': {'type': 'int', }, 'md5_value': {'type': 'str', }, 'encrypted': {'type': 'str', }},
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/interface/ethernet/{ethernet_ifnum}/ip/ospf/ospf-ip/{ip_addr}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ospf-ip"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["ospf-ip"].get(k) != v:
            change_results["changed"] = True
            config_changes["ospf-ip"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ospf-ip", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["ospf-ip"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ospf-ip-list"] if info != "NotFound" else info
