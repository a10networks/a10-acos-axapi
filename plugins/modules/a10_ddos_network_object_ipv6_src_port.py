...
DOCUMENTATION = r'''
module: a10_ddos_network_object_ipv6_src_port
description:
    ...
    host_src_port_anomaly_threshold:
        ...
        suboptions:
            host_src_port_pkt_rate:
                description:
                - "Forward packet rate of per-host source port entries"
                type: int
            host_src_port_bit_rate:
                description:
                - "Forward bit rate of per-host source port entries"
                type: int
    subnet_src_port_anomaly_threshold:
        ...
        suboptions:
            subnet_src_port_pkt_rate:
                description:
                - "Forward packet rate of per-subnet source port entries"
                type: int
            subnet_src_port_bit_rate:
                description:
                - "Forward bit rate of per-subnet source port entries"
                type: int
    uuid:
        description:
        - "uuid of the object"
...
AVAILABLE_PROPERTIES = ["host_src_port_anomaly_threshold", "port_num", "protocol", "subnet_src_port_anomaly_threshold", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(ipv6_subnet_ipv6_addr=dict(type='str', required=True), network_object_object_name=dict(type='str', required=True), ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object/{network_object_object_name}/ipv6/{ipv6_subnet_ipv6_addr}/src-port/{port_num}+{protocol}"
    ...
    if '/' in module.params["ipv6_subnet_ipv6_addr"]:
        f_dict["ipv6_subnet_ipv6_addr"] = module.params["ipv6_subnet_ipv6_addr"].replace("/", "%2F")
    else:
        f_dict["ipv6_subnet_ipv6_addr"] = module.params["ipv6_subnet_ipv6_addr"]
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object/{network_object_object_name}/ipv6/{ipv6_subnet_ipv6_addr}/src-port/"
