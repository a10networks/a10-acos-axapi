...
def get_argspec():
    ...
    rv.update({'ipv4': {'type': 'str', 'required': True, }, 'interface_ipv4': {'type': 'dict', 'ipv4_eth': {'type': 'str', }, 'ipv4_ve': {'type': 'int', }, 'ipv4_mgmt': {'type': 'bool', }}, 'uuid': {'type': 'str', }})
    return rv
...
def new_url(module):
    ...
    url_base = "/axapi/v3/network/lldp/management-address/ipv4-addr"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ipv4-addr"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["ipv4-addr"].get(k) != v:
            change_results["changed"] = True
            config_changes["ipv4-addr"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ipv4-addr", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["ipv4-addr"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ipv4-addr-list"] if info != "NotFound" else info
