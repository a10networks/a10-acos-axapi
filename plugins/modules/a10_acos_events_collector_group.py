...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'format': {'type': 'str', 'choices': ['syslog', 'cef', 'leef']},
        'facility': {'type': 'str', 'choices': ['local0', 'local1', 'local2', 'local3', 'local4', 'local5', 'local6', 'local7']},
        'health_check': {'type': 'str', },
        'rate': {'type': 'int', },
        'use_mgmt_port': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'msgs_sent', 'msgs_rate_limited', 'msgs_dropped']}},
        'log_server_list': {'type': 'list', 'name': {'type': 'str', 'required': True, }, 'port': {'type': 'int', 'required': True, }, 'uuid': {'type': 'str', }},
        'stats': {'type': 'dict', 'msgs_sent': {'type': 'str', }, 'msgs_rate_limited': {'type': 'str', }, 'msgs_dropped': {'type': 'str', }, 'name': {'type': 'str', 'required': True, }}
    })
    return rv
...
def present(module, result, existing_config):
    payload = utils.build_json("collector-group", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["collector-group"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["collector-group-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["collector-group"]["stats"] if info != "NotFound" else info
