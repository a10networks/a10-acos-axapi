...
options:
    ...
    sampling_enable:
        ...
        suboptions:
          ...
          Request; 'drop-vld-gtp-c-handover-in-progress'= Validation Drop= GTP-C Drop
          Since Handover In Progress; 'drop-vld-v0-reserved-message-drop'= Validation
          Drop= GTPv0-C Reserved Message Drop; 'drop-vld-v1-reserved-message-drop'=
          Validation Drop= GTPv1-C Reserved Message Drop; 'drop-vld-v2-reserved-message-
          drop'= Validation Drop= GTPv2-C Reserved Message Drop; 'drop-vld-invalid-pkt-
          len-piggyback'= Validation Drop= Piggyback message invalid packet length;
          'drop-vld-sanity-failed-piggyback'= Validation Drop= piggyback message anomaly
...
AVAILABLE_PROPERTIES = ["filtering_policy_name", "general_policy_name", "logging_policy_name", "name", "packet_capture_template", "rate_limit_policy_name", "sampling_enable", "stats", "user_tag", "uuid", "validation_policy_name", ]
...
def present(module, result, existing_config):
    payload = utils.build_json("gtp-policy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["gtp-policy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["gtp-policy-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["gtp-policy"]["stats"] if info != "NotFound" else info
