...
DOCUMENTATION = r'''
module: a10_health_monitor_method_imap
description:
    - IMAP type
...
def get_argspec():
    ...
    rv.update({'imap': {'type': 'bool', },
        'imap_port': {'type': 'int', },
        'imap_username': {'type': 'str', },
        'imap_password': {'type': 'bool', },
        'imap_password_string': {'type': 'str', },
        'imap_encrypted': {'type': 'str', },
        'pwd_auth': {'type': 'bool', },
        'imap_plain': {'type': 'bool', },
        'imap_cram_md5': {'type': 'bool', },
        'imap_login': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["imap"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["imap"].get(k) != v:
            change_results["changed"] = True
            config_changes["imap"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("imap", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["imap"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["imap-list"] if info != "NotFound" else info
