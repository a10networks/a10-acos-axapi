...
DOCUMENTATION = r'''
module: a10_ddos_notification_template_api
description:
...
options:
    ...
    relative_uri:
        description:
        - "Configure the relative uri for the api(e.g /example , default /) (api relative
          uri)"
        type: str
        required: False
...
AVAILABLE_PROPERTIES = ["authentication", "disable_authentication", "host_ipv4_address", "host_ipv6_address", "hostname", "http_port", "http_protocol", "https_port", "relative_uri", "timeout", "use_mgmt_port", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/notification-template/{notification_template_name}/api"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["api"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["api"].get(k) != v:
            change_results["changed"] = True
            config_changes["api"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("api", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["api"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["api-list"] if info != "NotFound" else info
