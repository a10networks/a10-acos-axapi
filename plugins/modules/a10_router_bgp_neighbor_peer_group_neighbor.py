...
def existing_url(module):
    ...
    f_dict = {}
    if '/' in str(module.params["peer_group"]):
        f_dict["peer_group"] = module.params["peer_group"].replace("/","%2F")
    else:
        f_dict["peer_group"] = module.params["peer_group"]
...
def present(module, result, existing_config):
    payload = utils.build_json("peer-group-neighbor", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["peer-group-neighbor"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["peer-group-neighbor-list"] if info != "NotFound" else info
