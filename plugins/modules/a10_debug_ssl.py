...
options:
    ...
    client_server:
        description:
        - "'clientside'= clientside SSL connection; 'serverside'= serverside SSL
          connection;"
        type: str
        required: False
    payload_dump_max:
        description:
        - "Application payloads exceeding this limit will be truncated to dump. Set to 0
          to disable payload dumping"
        type: int
        required: False
    payload_dump_string:
        description:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ssl"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["ssl"].get(k) != v:
            change_results["changed"] = True
            config_changes["ssl"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ssl", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["ssl"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ssl-list"] if info != "NotFound" else info
