...
DOCUMENTATION = r'''
module: a10_ddos_dst_entry_src_port_stats_dns_tcp_port
description:
    - Statistics for the object src-port
author: A10 Networks
options:
    state:
        description:
...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(protocol=dict(type='str', required=True), src_port_port_num=dict(type='str', required=True), entry_dst_entry_name=dict(type='str', required=True), ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/src-port/{src_port_port_num}+{protocol}/stats?dns-tcp-port=true"
    ...
    if '/' in module.params["src_port_port_num"]:
        f_dict["src_port_port_num"] = module.params["src_port_port_num"].replace("/", "%2F")
    else:
        f_dict["src_port_port_num"] = module.params["src_port_port_num"]
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["src-port"]["stats"] if info != "NotFound" else info
