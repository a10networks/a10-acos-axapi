...
options:
    ...
    service_config_template:
        description:
        - "Scaleout service-config template to bind (Scaleout service-config template
          name)"
        type: str
        required: False
    usable_nat_ports:
...
AVAILABLE_PROPERTIES = [
    "dest_rule_list", "dynamic_pool_size", "inside_end_address", "inside_netmask", "inside_start_address", "method", "nat_end_address", "nat_ip_list", "nat_netmask", "nat_start_address", "offset", "partition", "ports_per_user", "respond_to_user_mac", "service_config_template", "session_quota", "skip_ports_on_rollover", "usable_nat_ports", "uuid",
    "vrid",
    ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/cgnv6/fixed-nat/inside/ipv4address/{inside_start_address}+{inside_end_address}+{inside_netmask}+{partition}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ipv4address"].items():
        ...
        if config_changes["ipv4address"].get(k) != v:
            change_results["changed"] = True
            config_changes["ipv4address"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ipv4address", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["ipv4address"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ipv4address-list"] if info != "NotFound" else info
