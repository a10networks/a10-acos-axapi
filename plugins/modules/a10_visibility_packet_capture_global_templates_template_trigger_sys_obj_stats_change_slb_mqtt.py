...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates_template_trigger_sys_obj_stats_change_slb_mqtt
description:
    - Configure triggers for slb.mqtt object
...
def get_argspec():
    ...
    rv.update({'dummy': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'trigger_stats_inc': {'type': 'dict', 'parse_connect_fail': {'type': 'bool', }, 'parse_publish_fail': {'type': 'bool', }, 'parse_subscribe_fail': {'type': 'bool', }, 'parse_unsubscribe_fail': {'type': 'bool', }, 'tuple_not_linked': {'type': 'bool', }, 'tuple_already_linked': {'type': 'bool', }, 'conn_null': {'type': 'bool', }, 'client_id_null': {'type': 'bool', }, 'session_exist': {'type': 'bool', }, 'insertion_failed': {'type': 'bool', }, 'uuid': {'type': 'str', }},
        'trigger_stats_rate': {'type': 'dict', 'threshold_exceeded_by': {'type': 'int', }, 'duration': {'type': 'int', }, 'parse_connect_fail': {'type': 'bool', }, 'parse_publish_fail': {'type': 'bool', }, 'parse_subscribe_fail': {'type': 'bool', }, 'parse_unsubscribe_fail': {'type': 'bool', }, 'tuple_not_linked': {'type': 'bool', }, 'tuple_already_linked': {'type': 'bool', }, 'conn_null': {'type': 'bool', }, 'client_id_null': {'type': 'bool', }, 'session_exist': {'type': 'bool', }, 'insertion_failed': {'type': 'bool', }, 'uuid': {'type': 'str', }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/slb-mqtt"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["slb-mqtt"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["slb-mqtt"].get(k) != v:
            change_results["changed"] = True
            config_changes["slb-mqtt"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("slb-mqtt", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["slb-mqtt"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["slb-mqtt-list"] if info != "NotFound" else info
