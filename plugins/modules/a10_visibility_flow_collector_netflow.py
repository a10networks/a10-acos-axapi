...
def get_argspec():
    ...
    rv.update({'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'pkts-rcvd', 'v9-templates-created', 'v9-templates-deleted', 'v10-templates-created', 'v10-templates-deleted', 'template-drop-exceeded', 'template-drop-out-of-memory', 'frag-dropped', 'agent-not-found', 'version-not-supported', 'unknown-dir']}},
        'template': {'type': 'dict', 'uuid': {'type': 'str', }, 'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'templates-added-to-delq', 'templates-removed-from-delq']}}, 'detail': {'type': 'dict', 'uuid': {'type': 'str', }}},
        'stats': {'type': 'dict', 'pkts_rcvd': {'type': 'str', }, 'v9_templates_created': {'type': 'str', }, 'v9_templates_deleted': {'type': 'str', }, 'v10_templates_created': {'type': 'str', }, 'v10_templates_deleted': {'type': 'str', }, 'template_drop_exceeded': {'type': 'str', }, 'template_drop_out_of_memory': {'type': 'str', }, 'frag_dropped': {'type': 'str', }, 'agent_not_found': {'type': 'str', }, 'version_not_supported': {'type': 'str', }, 'unknown_dir': {'type': 'str', }, 'template': {'type': 'dict', 'stats': {'type': 'dict', 'templates_added_to_delq': {'type': 'str', }, 'templates_removed_from_delq': {'type': 'str', }}}}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["netflow"].items():
        ...
        if config_changes["netflow"].get(k) != v:
            change_results["changed"] = True
            config_changes["netflow"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("netflow", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["netflow"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["netflow-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["netflow"]["stats"] if info != "NotFound" else info
