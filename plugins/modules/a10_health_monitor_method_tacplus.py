...
def get_argspec():
    ...
    rv.update({'tacplus': {'type': 'bool', },
        'tacplus_username': {'type': 'str', },
        'tacplus_password': {'type': 'bool', },
        'tacplus_password_string': {'type': 'str', },
        'tacplus_encrypted': {'type': 'str', },
        'tacplus_secret': {'type': 'bool', },
        'tacplus_secret_string': {'type': 'str', },
        'secret_encrypted': {'type': 'str', },
        'tacplus_port': {'type': 'int', },
        'tacplus_type': {'type': 'str', 'choices': ['inbound-ascii-login']},
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["tacplus"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["tacplus"].get(k) != v:
            change_results["changed"] = True
            config_changes["tacplus"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("tacplus", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["tacplus"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["tacplus-list"] if info != "NotFound" else info
