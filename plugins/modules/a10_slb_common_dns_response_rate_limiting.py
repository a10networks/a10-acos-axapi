...
def get_argspec():
    ...
    rv.update({'max_table_entries': {'type': 'int', },
        'uuid': {'type': 'str', }
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/slb/common/dns-response-rate-limiting"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-response-rate-limiting"].items():
        ...
        if config_changes["dns-response-rate-limiting"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-response-rate-limiting"][k] = v
