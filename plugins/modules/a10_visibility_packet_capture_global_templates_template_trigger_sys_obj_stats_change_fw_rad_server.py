...
def get_argspec():
    ...
    rv.update({'dummy': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'trigger_stats_inc': {'type': 'dict', 'radius_table_full': {'type': 'bool', }, 'ipv6_prefix_length_mismatch': {'type': 'bool', }, 'uuid': {'type': 'str', }},
        'trigger_stats_rate': {'type': 'dict', 'threshold_exceeded_by': {'type': 'int', }, 'duration': {'type': 'int', }, 'radius_table_full': {'type': 'bool', }, 'ipv6_prefix_length_mismatch': {'type': 'bool', }, 'uuid': {'type': 'str', }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/fw-rad-server"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["fw-rad-server"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["fw-rad-server"].get(k) != v:
            change_results["changed"] = True
            config_changes["fw-rad-server"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("fw-rad-server", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["fw-rad-server"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["fw-rad-server-list"] if info != "NotFound" else info
