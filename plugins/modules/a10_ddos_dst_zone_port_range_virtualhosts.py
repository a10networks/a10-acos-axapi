...
DOCUMENTATION = r'''
module: a10_ddos_dst_zone_port_range_virtualhosts
description:
    - Configure virtualhost based mitigation for SSL services
author: A10 Networks
options:
    state:
        description:
...
AVAILABLE_PROPERTIES = ["source_tracking_all", "uuid", "vhosts_config", "virtualhost_list", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/zone/{zone_name}/port-range/{port_range_port_range_start}+{port_range_end}+{protocol}/virtualhosts"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["virtualhosts"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["virtualhosts"].get(k) != v:
            change_results["changed"] = True
            config_changes["virtualhosts"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("virtualhosts", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["virtualhosts"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["virtualhosts-list"] if info != "NotFound" else info
