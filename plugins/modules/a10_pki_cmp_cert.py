...
options:
    ...
    url:
        description:
        - "CMP server's absolute URL(http(s)=//host=[port]/path), path is the location to
          use for the CMP server(aka CMP alias)"
        type: str
        required: False
    subject_dn:
        description:
        - "Distinguished Name to use while enrolling the certificate(For EJBCA CA, this is
          the subject DN of an End Entity) (DN OID is case sensitive)"
        type: str
        required: False
    recipient_dn:
        description:
        - "Distinguished Name of the CMP message recipient, i.e., the CMP server (usually
          a CA or RA entity)) (DN OIDis case sensitive)"
        type: str
        required: False
    ...
    cmp_trusted_cert:
        description:
        - "The specific CMP server certificate to use and directly trust when verifying
          signature of CMP response message"
        type: str
        required: False
    allow_unprotected_errors:
        description:
        - "Accept missing or invalid protection of negative responses(CA likes EJCBA tends
          to not protect negative responses)"
        type: bool
        required: False
    uuid:
        description:
...
AVAILABLE_PROPERTIES = ["allow_unprotected_errors", "cert_type", "cmp_trusted_ca", "cmp_trusted_cert", "ec_key_length", "ecdsa_type", "encrypted", "enroll", "log_level", "max_polltime", "minute", "name", "recipient_dn", "renew_before", "renew_before_type", "renew_before_value", "renew_every", "renew_every_type", "renew_every_value", "rsa_key_length", "rsa_type", "secret", "secret_string", "subject_alternate_name", "subject_dn", "url", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        'url': {'type': 'str', },
        'subject_dn': {'type': 'str', },
        'recipient_dn': {'type': 'str', },
        'subject_alternate_name': {'type': 'dict', 'san_type': {'type': 'str', 'choices': ['email', 'dns', 'ip']}, 'san_value': {'type': 'str', }},
        'enroll': {'type': 'bool', },
        'log_level': {'type': 'int', },
        'secret': {'type': 'bool', },
        ...
        'rsa_key_length': {'type': 'str', 'choices': ['1024', '2048', '4096', '8192']},
        'ec_key_length': {'type': 'str', 'choices': ['256', '384']},
        'max_polltime': {'type': 'int', },
        'cmp_trusted_ca': {'type': 'str', },
        'cmp_trusted_cert': {'type': 'str', },
        'allow_unprotected_errors': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', }
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/pki/cmp-cert/{name}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["cmp-cert"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["cmp-cert"].get(k) != v:
            change_results["changed"] = True
            config_changes["cmp-cert"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("cmp-cert", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["cmp-cert"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["cmp-cert-list"] if info != "NotFound" else info
