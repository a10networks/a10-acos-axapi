...
options:
    ...
    sampling_enable:
        ...
        suboptions:
          ...
          Total Current Established Connections; 'l4-bandwidth'= L4 Bandwidth in
          bits/sec; 'l7-bandwidth'= L7 Bandwidth in bits/sec; 'serv-ssl-conns-per-sec'=
          Server SSL Connections/sec; 'fw-conns-per-sec'= FW Connections/sec; 'gifw-
          conns-per-sec'= GiFW Connections/sec;"
                type: str
    stats:
        description:
        - "Field stats"
...
def get_argspec():
    ...
    rv.update({'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'total-throughput-bits-per-sec', 'l4-conns-per-sec', 'l7-conns-per-sec', 'l7-trans-per-sec', 'ssl-conns-per-sec', 'ip-nat-conns-per-sec', 'total-new-conns-per-sec', 'total-curr-conns', 'l4-bandwidth', 'l7-bandwidth', 'serv-ssl-conns-per-sec', 'fw-conns-per-sec', 'gifw-conns-per-sec']}},
        'stats': {'type': 'dict', 'total_throughput_bits_per_sec': {'type': 'str', }, 'l4_conns_per_sec': {'type': 'str', }, 'l7_conns_per_sec': {'type': 'str', }, 'l7_trans_per_sec': {'type': 'str', }, 'ssl_conns_per_sec': {'type': 'str', }, 'ip_nat_conns_per_sec': {'type': 'str', }, 'total_new_conns_per_sec': {'type': 'str', }, 'total_curr_conns': {'type': 'str', }, 'l4_bandwidth': {'type': 'str', }, 'l7_bandwidth': {'type': 'str', }, 'serv_ssl_conns_per_sec': {'type': 'str', }, 'fw_conns_per_sec': {'type': 'str', }, 'gifw_conns_per_sec': {'type': 'str', }}
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/system/app-performance"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["app-performance"].items():
        ...
        if config_changes["app-performance"].get(k) != v:
            change_results["changed"] = True
            config_changes["app-performance"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("app-performance", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["app-performance"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["app-performance-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["app-performance"]["stats"] if info != "NotFound" else info
