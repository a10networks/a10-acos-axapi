...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates_template_trigger_sys_obj_stats_change_system_hardware_accelerate
description:
    - Configure triggers for system.hardware-accelerate object
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/system-hardware-accelerate"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["system-hardware-accelerate"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["system-hardware-accelerate"].get(k) != v:
            change_results["changed"] = True
            config_changes["system-hardware-accelerate"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("system-hardware-accelerate", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["system-hardware-accelerate"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["system-hardware-accelerate-list"] if info != "NotFound" else info
