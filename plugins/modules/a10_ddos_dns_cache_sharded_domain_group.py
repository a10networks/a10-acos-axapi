...
DOCUMENTATION = r'''
module: a10_ddos_dns_cache_sharded_domain_group
description:
...
AVAILABLE_PROPERTIES = ["encap_template", "match_action", "name", "sharded_domain_list_policy_list", "user_tag", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dns-cache/{dns_cache_name}/sharded-domain-group/{name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/dns-cache/{dns_cache_name}/sharded-domain-group"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["sharded-domain-group"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["sharded-domain-group"].get(k) != v:
            change_results["changed"] = True
            config_changes["sharded-domain-group"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("sharded-domain-group", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["sharded-domain-group"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["sharded-domain-group-list"] if info != "NotFound" else info
