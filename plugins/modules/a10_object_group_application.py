...
def existing_url(module):
    ...
    url_base = "/axapi/v3/object-group/application/{app_name}"

    f_dict = {}
    if '/' in str(module.params["app_name"]):
        f_dict["app_name"] = module.params["app_name"].replace("/","%2F")
    else:
        f_dict["app_name"] = module.params["app_name"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["application"].items():
        ...
        if config_changes["application"].get(k) != v:
            change_results["changed"] = True
            config_changes["application"][k] = v
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["application"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["application-list"] if info != "NotFound" else info
