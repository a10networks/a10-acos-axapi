...
DOCUMENTATION = r'''
module: a10_waf_template_json_check
description:
    - JSON check
...
AVAILABLE_PROPERTIES = ["format_check", "max_array_values", "max_depth", "max_object_members", "max_string_length", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'format_check': {'type': 'bool', }, 'max_array_values': {'type': 'int', }, 'max_depth': {'type': 'int', }, 'max_object_members': {'type': 'int', }, 'max_string_length': {'type': 'int', }, 'uuid': {'type': 'str', }})
    # Parent keys
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/waf/template/{template_name}/json-check"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["json-check"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["json-check"].get(k) != v:
            change_results["changed"] = True
            config_changes["json-check"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("json-check", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["json-check"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["json-check-list"] if info != "NotFound" else info
