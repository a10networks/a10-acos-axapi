...
DOCUMENTATION = r'''
module: a10_sys_ut_state_next_state_case_action_tcp_options
description:
    - Tcp Flags
author: A10 Networks
options:
...
def get_argspec():
    ...
    rv.update({'mss': {'type': 'int', }, 'wscale': {'type': 'int', }, 'sack_type': {'type': 'str', 'choices': ['permitted', 'block']}, 'time_stamp_enable': {'type': 'bool', }, 'nop': {'type': 'bool', }, 'uuid': {'type': 'str', }})
    # Parent keys
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["options"].items():
        ...
        if config_changes["options"].get(k) != v:
            change_results["changed"] = True
            config_changes["options"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("options", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["options"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["options-list"] if info != "NotFound" else info
