...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["gtp-filter-list"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["gtp-filter-list"].get(k) != v:
            change_results["changed"] = True
            config_changes["gtp-filter-list"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("gtp-filter-list", module.params, AVAILABLE_PROPERTIES)
