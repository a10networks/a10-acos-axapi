...
AVAILABLE_PROPERTIES = ["action", "dst_file", "file", "file_handle", "oper", "skip_backup", "uuid", ]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["aflex"].items():
        if k not in file_check:
            continue
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["aflex"].get(k) != v:
            change_results["changed"] = True
            config_changes["aflex"][k] = v
...
def run_command(module):
    ...
    try:
        ...
        existing_config, file_exists = api_client.get_file(module.client, "aflex", file_url, module.params['file'])
