...
short_description: Configures A10 overlay-tunnel.vtep.destination.ip.address.vni
...
options:
    ...
    destination_ip_address_ip_address:
        description:
        - Key to identify parent object    vtep_id:
        description:
        - Key to identify parent object    segment:
        description:
...
def get_argspec():
    ...
    rv.update(
        dict(
            destination_ip_address_ip_address=dict(type='str', required=True),
...
def existing_url(module):
    ...
    f_dict["destination_ip_address_ip_address"] = module.params[
        "destination_ip_address_ip_address"]
    ...
    if existing_config:
        for k, v in payload["vni"].items():
            if isinstance(v, str):
                ...
                if existing_config["vni"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["vni"][k] = v
