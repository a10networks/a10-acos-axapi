...
def existing_url(module):
    ...
    if '/' in str(module.params["trunk_number"]):
        f_dict["trunk_number"] = module.params["trunk_number"].replace(
            "/", "%2F")
    else:
        f_dict["trunk_number"] = module.params["trunk_number"]
    if '/' in module.params["ethernet_ifnum"]:
        f_dict["ethernet_ifnum"] = module.params["ethernet_ifnum"].replace(
            "/", "%2F")
    else:
...
def present(module, result, existing_config):
    payload = utils.build_json("trunk-group", module.params,
                               AVAILABLE_PROPERTIES)
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info[
                    "trunk-group"] if info != "NotFound" else info
                ...
                result["acos_info"] = info[
                    "trunk-group-list"] if info != "NotFound" else info
