...
short_description: Configures A10 cgnv6.map.encapsulation.domain.health-check-gateway
...
options:
    ...
    domain_name:
        description:
        - Key to identify parent object    ipv6_address_list:
        description:
    ...
    if existing_config:
        for k, v in payload["health-check-gateway"].items():
            if isinstance(v, str):
                ...
                if existing_config["health-check-gateway"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["health-check-gateway"][k] = v
