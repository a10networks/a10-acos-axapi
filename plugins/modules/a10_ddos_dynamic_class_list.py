...
DOCUMENTATION = r'''
module: a10_ddos_dynamic_class_list
description:
    - Configure class-list for automatic population by BGP
author: A10 Networks
options:
    state:
        description:
...
AVAILABLE_PROPERTIES = ["class_list_name", "oper", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dynamic-class-list"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dynamic-class-list"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["dynamic-class-list"].get(k) != v:
            change_results["changed"] = True
            config_changes["dynamic-class-list"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dynamic-class-list", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dynamic-class-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dynamic-class-list-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["dynamic-class-list"]["oper"] if info != "NotFound" else info
