...
DOCUMENTATION = r'''
module: a10_ddos_dst_zone_port_zone_service_virtualhosts_virtualhost_level
description:
    - Virtualhost Policy Level Configuration
author: A10 Networks
options:
    state:
        description:
...
AVAILABLE_PROPERTIES = ["glid_action", "level_num", "src_default_glid", "user_tag", "uuid", "zone_template", ]
...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(virtualhost_vhost=dict(type='str', required=True), protocol=dict(type='str', required=True), zone_service_port_num=dict(type='str', required=True), zone_name=dict(type='str', required=True), ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/zone/{zone_name}/port/zone-service/{zone_service_port_num}+{protocol}/virtualhosts/virtualhost/{virtualhost_vhost}/level/{level_num}"
    ...
    if '/' in module.params["virtualhost_vhost"]:
        f_dict["virtualhost_vhost"] = module.params["virtualhost_vhost"].replace("/", "%2F")
    else:
        f_dict["virtualhost_vhost"] = module.params["virtualhost_vhost"]
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/zone/{zone_name}/port/zone-service/{zone_service_port_num}+{protocol}/virtualhosts/virtualhost/{virtualhost_vhost}/level"
