...
options:
    ...
    sampling_enable:
        ...
        suboptions:
            counters1:
                description:
                - "'all'= all; 'req'= Request; 'req-reject'= Request Rejected; 'req-auth'= Request
...
AVAILABLE_PROPERTIES = ["aaa_rule_list", "name", "packet_capture_template", "sampling_enable", "stats", "user_tag", "uuid", ]
...
def get_argspec():
    ...
    rv.update({
        ...
        'sampling_enable': {
            ...
            'counters1': {
                'type': 'str',
                'choices': ['all', 'req', 'req-reject', 'req-auth', 'req-bypass', 'req-skip', 'error', 'failure-bypass']
                }
            },
        'packet_capture_template': {
            'type': 'str',
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["aaa-policy"].items():
        ...
        if config_changes["aaa-policy"].get(k) != v:
            change_results["changed"] = True
            config_changes["aaa-policy"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("aaa-policy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["aaa-policy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["aaa-policy-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["aaa-policy"]["stats"] if info != "NotFound" else info
