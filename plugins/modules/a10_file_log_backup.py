...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["log-backup"].items():
        ...
        if config_changes["log-backup"].get(k) != v:
            change_results["changed"] = True
            config_changes["log-backup"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("log-backup", module.params, AVAILABLE_PROPERTIES)
...
def run_command(module):
    ...
    try:
        ...
        if state == 'present' or state == 'absent':
            ...
            existing_config, file_exists = api_client.get_file(module.client, "log-backup", file_url, module.params['file'])
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["log-backup"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["log-backup-list"] if info != "NotFound" else info
