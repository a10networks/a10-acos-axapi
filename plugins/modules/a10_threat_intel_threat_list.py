...
def get_argspec():
    ...
    rv.update({
        ...
        'sampling_enable': {
            ...
            'counters1': {
                'type': 'str',
                'choices': ['all', 'spam-sources', 'windows-exploits', 'web-attacks', 'botnets', 'scanners', 'dos-attacks', 'reputation', 'phishing', 'proxy', 'mobile-threats', 'tor-proxy', 'total-hits']
                }
            },
        'stats': {
            'type': 'dict',
...
def new_url(module):
    ...
    url_base = "/axapi/v3/threat-intel/threat-list"
...
def present(module, result, existing_config):
    payload = utils.build_json("threat-list", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["threat-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["threat-list-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["threat-list"]["stats"] if info != "NotFound" else info
