...
short_description: Configures A10 sys.ut.event.action.ignore-validation
    ...
    if existing_config:
        for k, v in payload["ignore-validation"].items():
            if isinstance(v, str):
                ...
                if existing_config["ignore-validation"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["ignore-validation"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("ignore-validation", module)
