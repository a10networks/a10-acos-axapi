...
def get_argspec():
    ...
    rv.update({'l1': {'type': 'bool', }, 'l2': {'type': 'bool', }, 'l3': {'type': 'bool', }, 'l4': {'type': 'bool', }, 'all': {'type': 'bool', }, 'uuid': {'type': 'str', }})
    # Parent keys
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["ignore-validation"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["ignore-validation"].get(k) != v:
            change_results["changed"] = True
            config_changes["ignore-validation"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("ignore-validation", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["ignore-validation"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["ignore-validation-list"] if info != "NotFound" else info
