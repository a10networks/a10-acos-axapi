...
def existing_url(module):
    ...
    if '/' in str(module.params["disable_ipv6_support"]):
        f_dict["disable_ipv6_support"] = module.params[
            "disable_ipv6_support"].replace("/", "%2F")
    else:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["src-ip"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["src-ip"].get(k) != v:
            change_results["changed"] = True
            config_changes["src-ip"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("src-ip", module.params, AVAILABLE_PROPERTIES)
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info[
                    "src-ip"] if info != "NotFound" else info
                ...
                result["acos_info"] = info[
                    "src-ip-list"] if info != "NotFound" else info
