...
short_description: Configures A10 acos.events.message.id.property.severity
...
options:
    ...
    message_id_log_msg:
        description:
        - Key to identify parent object    severity_val:
        description:
    ...
    if existing_config:
        for k, v in payload["severity"].items():
            if isinstance(v, str):
                ...
                if existing_config["severity"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["severity"][k] = v
