...
def get_argspec():
    ...
    rv.update({'file_path': {'type': 'str', },'file': {'type': 'str', },
        'action': {'type': 'str', 'choices': ['create', 'check', 'delete']}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["techsupport"].items():
        if k not in file_check:
            continue
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["techsupport"].get(k) != v:
            change_results["changed"] = True
            config_changes["techsupport"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("techsupport", module.params, AVAILABLE_PROPERTIES)
...
def run_command(module):
    ...
    try:
        ...
        existing_config, file_exists = api_client.get_file(
            module.client, "techsupport", file_url,
            module.params['file'])
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["techsupport"] if info != "NotFound" else info
