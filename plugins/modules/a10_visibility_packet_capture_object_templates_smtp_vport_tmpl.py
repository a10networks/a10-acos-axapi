...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_object_templates_smtp_vport_tmpl
description:
    - Configure template for counter.smtp_vport
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/object-templates/smtp-vport-tmpl/{name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/object-templates/smtp-vport-tmpl"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["smtp-vport-tmpl"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["smtp-vport-tmpl"].get(k) != v:
            change_results["changed"] = True
            config_changes["smtp-vport-tmpl"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("smtp-vport-tmpl", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["smtp-vport-tmpl"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["smtp-vport-tmpl-list"] if info != "NotFound" else info
