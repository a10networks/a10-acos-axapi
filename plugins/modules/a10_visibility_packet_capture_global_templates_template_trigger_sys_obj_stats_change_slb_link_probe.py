...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates_template_trigger_sys_obj_stats_change_slb_link_probe
description:
    - Configure triggers for slb.link-probe.entry object
    ...
    trigger_stats_inc:
        ...
        suboptions:
            ...
            total_http_response_bad:
                description:
                - "Enable automatic packet-capture for Total HTTP responses not matching probe
          template config"
                type: bool
            total_tcp_err:
                description:
            ...
            err_probe_tcp_conn_send:
                description:
                - "Enable automatic packet-capture for Error in initiating TCP connection for
          probe"
                type: bool
            uuid:
                description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/slb-link-probe"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["slb-link-probe"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["slb-link-probe"].get(k) != v:
            change_results["changed"] = True
            config_changes["slb-link-probe"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("slb-link-probe", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["slb-link-probe"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["slb-link-probe-list"] if info != "NotFound" else info
