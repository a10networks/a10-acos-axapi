...
def get_argspec():
    ...
    rv.update({'ip_address': {'type': 'str', 'required': True, },
        'uuid': {'type': 'str', },
        'vni_list': {'type': 'list', 'segment': {'type': 'int', 'required': True, }, 'partition': {'type': 'str', }, 'gateway': {'type': 'bool', }, 'lif': {'type': 'int', }, 'uuid': {'type': 'str', }}
    })
    # Parent keys
    rv.update(dict(
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["source-ip-address"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["source-ip-address"].get(k) != v:
            change_results["changed"] = True
            config_changes["source-ip-address"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("source-ip-address", module.params, AVAILABLE_PROPERTIES)
