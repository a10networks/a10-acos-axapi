...
def get_argspec():
    ...
    rv.update({'connection_load_enable': {'type': 'bool', },
        'connection_load_fail_break': {'type': 'bool', },
        'connection_load_samples': {'type': 'int', },
        'connection_load_interval': {'type': 'int', },
        'limit': {'type': 'bool', },
        'connection_load_limit': {'type': 'int', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["connection-load"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["connection-load"].get(k) != v:
            change_results["changed"] = True
            config_changes["connection-load"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("connection-load", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["connection-load"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["connection-load-list"] if info != "NotFound" else info
