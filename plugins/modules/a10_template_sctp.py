...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        'sctp_half_open_idle_timeout': {'type': 'int', },
        'sctp_idle_timeout': {'type': 'int', },
        'permit_payload_protocol': {'type': 'dict', 'permit_config_id': {'type': 'list', 'protocol_id': {'type': 'int', }}, 'permit_config_name': {'type': 'list', 'protocol_name': {'type': 'str', 'choices': ['iua', 'm2ua', 'm3ua', 'sua', 'm2pa', 'h.323']}}},
        'checksum_check': {'type': 'str', 'choices': ['enable']},
        'log': {'type': 'dict', 'payload_proto_filtering': {'type': 'bool', }},
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', }
    })
    return rv
...
def present(module, result, existing_config):
    payload = utils.build_json("sctp", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["sctp"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["sctp-list"] if info != "NotFound" else info
