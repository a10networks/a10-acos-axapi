...
def get_argspec():
    ...
    rv.update({'running_config': {'type': 'bool', },
        'startup_config': {'type': 'bool', },
        'profile': {'type': 'str', },
        'use_mgmt_port': {'type': 'bool', },
        'remote_file': {'type': 'str', },
        'dest_profile': {'type': 'str', },
        'dest_use_mgmt_port': {'type': 'bool', },
        'dest_remote_file': {'type': 'str', },
        'to_running_config': {'type': 'bool', },
        'to_startup_config': {'type': 'bool', },
        'to_profile': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["copy"].items():
        ...
        if config_changes["copy"].get(k) != v:
            change_results["changed"] = True
            config_changes["copy"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("copy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["copy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["copy-list"] if info != "NotFound" else info
