...
def get_argspec():
    ...
    rv.update({'uuid': {'type': 'str', },
        ...
        'oper': {'type': 'dict', 'category': {'type': 'str', }, 'contains': {'type': 'str', }, 'related': {'type': 'str', }, 'group_list': {'type': 'list', 'category_name': {'type': 'str', }, 'app_name': {'type': 'str', }, 'app_desc': {'type': 'str', }}},
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["app"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["app"].get(k) != v:
            change_results["changed"] = True
            config_changes["app"][k] = v
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["app"]["oper"] if info != "NotFound" else info
