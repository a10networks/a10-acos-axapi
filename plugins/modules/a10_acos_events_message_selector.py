...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', },
        'rule_list': {'type': 'list', 'index': {'type': 'int', 'required': True, }, 'action': {'type': 'str', 'choices': ['send', 'drop']}, 'message_id': {'type': 'str', }, 'message_id_scope': {'type': 'str', 'choices': ['all', 'node-only', 'children-only', 'log-field-only']}, 'severity_oper': {'type': 'str', 'choices': ['equal-and-higher', 'equal']}, 'severity_val': {'type': 'str', 'choices': ['emergency', 'alert', 'critical', 'error', 'warning', 'notification', 'information', 'debugging']}, 'uuid': {'type': 'str', }, 'user_tag': {'type': 'str', }}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["message-selector"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["message-selector"].get(k) != v:
            change_results["changed"] = True
            config_changes["message-selector"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("message-selector", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["message-selector"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["message-selector-list"] if info != "NotFound" else info
