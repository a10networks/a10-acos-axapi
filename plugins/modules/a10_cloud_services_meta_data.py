...
def get_argspec():
    ...
    rv.update({'action': {'type': 'str', 'choices': ['enable', 'disable']},
        'provider': {'type': 'str', 'choices': ['aws', 'openstack']},
        'prevent_autofill': {'type': 'bool', },
        'prevent_admin_passwd': {'type': 'bool', },
        'prevent_admin_ssh_key': {'type': 'bool', },
        'prevent_user_ops': {'type': 'bool', },
        'prevent_blob': {'type': 'bool', },
        'prevent_license': {'type': 'bool', },
        'prevent_webservice': {'type': 'bool', },
        'prevent_cloud_service': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["meta-data"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["meta-data"].get(k) != v:
            change_results["changed"] = True
            config_changes["meta-data"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("meta-data", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["meta-data"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["meta-data-list"] if info != "NotFound" else info
