...
DOCUMENTATION = r'''
module: a10_slb_common_cert_pinning
description:
...
AVAILABLE_PROPERTIES = ["candidate_list_feedback_opt_in", "ttl", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/slb/common/cert-pinning"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["cert-pinning"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["cert-pinning"].get(k) != v:
            change_results["changed"] = True
            config_changes["cert-pinning"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("cert-pinning", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["cert-pinning"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["cert-pinning-list"] if info != "NotFound" else info
