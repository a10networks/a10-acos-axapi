...
def get_argspec():
    ...
    rv.update({
        ...
        'sampling_enable': {
            ...
            'counters1': {
                'type': 'str',
                'choices': ['all', 'total_playerids_created', 'total_playerids_deleted', 'total_abs_max_age_outs', 'total_pkt_activity_age_outs', 'total_invalid_playerid_pkts', 'total_invalid_playerid_drops', 'total_valid_playerid_pkts']
                }
            },
        'oper': {
            'type': 'dict',
...
def present(module, result, existing_config):
    payload = utils.build_json("player-id-global", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["player-id-global"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["player-id-global-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["player-id-global"]["oper"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["player-id-global"]["stats"] if info != "NotFound" else info
