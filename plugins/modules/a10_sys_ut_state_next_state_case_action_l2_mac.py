...
def get_argspec():
    ...
    rv.update({'src_dst': {'type': 'str', 'required': True, 'choices': ['dest', 'src']},
        'address_type': {'type': 'str', 'choices': ['broadcast', 'multicast']},
        'virtual_server': {'type': 'str', },
        'nat_pool': {'type': 'str', },
        'ethernet': {'type': 'str', },
        've': {'type': 'str', },
        'trunk': {'type': 'str', },
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/sys-ut/state/{state_name}/next-state/{name}/case/{case_number}/action/{action_direction}/l2/mac/{src_dst}"

    f_dict = {}
    if '/' in str(module.params["src_dst"]):
        f_dict["src_dst"] = module.params["src_dst"].replace("/","%2F")
    else:
        f_dict["src_dst"] = module.params["src_dst"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["mac"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["mac"].get(k) != v:
            change_results["changed"] = True
            config_changes["mac"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("mac", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["mac"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["mac-list"] if info != "NotFound" else info
