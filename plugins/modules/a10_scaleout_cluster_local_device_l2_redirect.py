...
def get_argspec():
    ...
    rv.update({'redirect_eth': {'type': 'str', }, 'ethernet_vlan': {'type': 'int', }, 'redirect_trunk': {'type': 'int', }, 'trunk_vlan': {'type': 'int', }, 'uuid': {'type': 'str', }})
    # Parent keys
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["l2-redirect"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["l2-redirect"].get(k) != v:
            change_results["changed"] = True
            config_changes["l2-redirect"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("l2-redirect", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["l2-redirect"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["l2-redirect-list"] if info != "NotFound" else info
