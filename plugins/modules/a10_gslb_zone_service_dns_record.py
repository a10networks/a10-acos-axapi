...
def get_argspec():
    ...
    rv.update({'ntype': {'type': 'int', 'required': True, }, 'data': {'type': 'str', }, 'uuid': {'type': 'str', }})
    # Parent keys
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-record/{type}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-record"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-record"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["dns-record"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-record"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-record", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dns-record"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dns-record-list"] if info != "NotFound" else info
