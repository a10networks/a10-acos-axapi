...
def get_argspec():
    ...
    rv.update({'num': {'type': 'int', 'required': True, },
        'conn_limit': {'type': 'int', },
        'conn_rate_limit': {'type': 'int', },
        'conn_rate_limit_interval': {'type': 'int', },
        'request_limit': {'type': 'int', },
        'request_rate_limit': {'type': 'int', },
        'request_rate_limit_interval': {'type': 'int', },
        'over_limit_action': {'type': 'bool', },
        'action_value': {'type': 'str', 'choices': ['drop', 'dns-cache-disable', 'dns-cache-enable', 'forward', 'reset']},
        'lockout': {'type': 'int', },
        'log': {'type': 'bool', },
        'log_interval': {'type': 'int', },
        'dns': {'type': 'dict', 'action': {'type': 'str', 'choices': ['cache-disable', 'cache-enable']}, 'weight': {'type': 'int', }, 'ttl': {'type': 'int', }},
        'dns64': {'type': 'dict', 'disable': {'type': 'bool', }, 'exclusive_answer': {'type': 'bool', }, 'prefix': {'type': 'str', }},
        'use_nat_pool': {'type': 'str', },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["glid"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["glid"].get(k) != v:
            change_results["changed"] = True
            config_changes["glid"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("glid", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["glid"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["glid-list"] if info != "NotFound" else info
