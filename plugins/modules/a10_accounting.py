...
def get_argspec():
    ...
    rv.update({'commands': {'type': 'int', },
        'debug': {'type': 'int', },
        'stop_only': {'type': 'bool', },
        'tacplus': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'nexec': {'type': 'dict', 'accounting_exec_type': {'type': 'str', 'choices': ['start-stop', 'stop-only']}, 'accounting_exec_method': {'type': 'str', 'choices': ['tacplus', 'radius']}, 'uuid': {'type': 'str', }}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["accounting"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["accounting"].get(k) != v:
            change_results["changed"] = True
            config_changes["accounting"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("accounting", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["accounting"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["accounting-list"] if info != "NotFound" else info
