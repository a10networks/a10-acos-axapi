...
def get_argspec():
    ...
    rv.update({'event_number': {'type': 'int', 'required': True, },
        'uuid': {'type': 'str', },
        'user_tag': {'type': 'str', },
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/sys-ut/event/{event_number}"

    f_dict = {}
    if '/' in str(module.params["event_number"]):
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["event"].items():
        ...
        if config_changes["event"].get(k) != v:
            change_results["changed"] = True
            config_changes["event"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("event", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["event"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["event-list"] if info != "NotFound" else info
