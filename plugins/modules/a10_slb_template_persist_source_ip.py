...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'enforce_higher_priority': {'type': 'bool', },
        'hash_persist': {'type': 'bool', },
        'incl_dst_ip': {'type': 'bool', },
        'incl_sport': {'type': 'bool', },
        'timeout': {'type': 'int', },
        'netmask': {'type': 'str', },
        'netmask6': {'type': 'int', },
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["source-ip"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["source-ip"].get(k) != v:
            change_results["changed"] = True
            config_changes["source-ip"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("source-ip", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["source-ip"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["source-ip-list"] if info != "NotFound" else info
