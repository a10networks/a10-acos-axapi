...
AVAILABLE_PROPERTIES = ["action", "authentication", "debug_mode", "host_name", "http_port", "https_port", "ipv4_address", "ipv6_address", "name", "protocol", "relative_uri", "sampling_enable", "stats", "test_connectivity", "use_mgmt_port", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'http_port': {'type': 'int', },
        'https_port': {'type': 'int', },
        'relative_uri': {'type': 'str', },
        'action': {'type': 'str', 'choices': ['enable', 'disable']},
        'debug_mode': {'type': 'bool', },
        'test_connectivity': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'sent_successful', 'send_fail', 'response_fail']}},
        'authentication': {'type': 'dict', 'relative_login_uri': {'type': 'str', }, 'relative_logoff_uri': {'type': 'str', }, 'auth_username': {'type': 'str', }, 'auth_password': {'type': 'bool', }, 'auth_password_string': {'type': 'str', }, 'encrypted': {'type': 'str', }, 'api_key': {'type': 'bool', }, 'api_key_string': {'type': 'str', }, 'api_key_encrypted': {'type': 'str', }, 'uuid': {'type': 'str', }},
        'stats': {'type': 'dict', 'sent_successful': {'type': 'str', }, 'send_fail': {'type': 'str', }, 'response_fail': {'type': 'str', }, 'name': {'type': 'str', 'required': True, }}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["template-name"].items():
        ...
        if config_changes["template-name"].get(k) != v:
            change_results["changed"] = True
            config_changes["template-name"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("template-name", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["template-name"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["template-name-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["template-name"]["stats"] if info != "NotFound" else info
