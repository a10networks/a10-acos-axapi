...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates
description:
    - Configure global packet capture templates for T1 counters
author: A10 Networks
options:
    state:
        description:
...
AVAILABLE_PROPERTIES = ["activate", "dummy", "template_list", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["global-templates"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["global-templates"].get(k) != v:
            change_results["changed"] = True
            config_changes["global-templates"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("global-templates", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["global-templates"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["global-templates-list"] if info != "NotFound" else info
