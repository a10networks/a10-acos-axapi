...
AVAILABLE_PROPERTIES = ["action", "dest_class_list", "priority", "sampling_enable", "stats", "ntype", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'dest_class_list': {'type': 'str', 'required': True, },
        'action': {'type': 'str', },
        'ntype': {'type': 'str', 'choices': ['host', 'url', 'ip']},
        'priority': {'type': 'int', },
        'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'hits']}},
        'stats': {'type': 'dict', 'hits': {'type': 'str', }, 'dest_class_list': {'type': 'str', 'required': True, }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/slb/template/policy/{policy_name}/forward-policy/source/{name}/destination/class-list/{dest_class_list}"

    f_dict = {}
    if '/' in str(module.params["dest_class_list"]):
        f_dict["dest_class_list"] = module.params["dest_class_list"].replace("/","%2F")
    else:
        f_dict["dest_class_list"] = module.params["dest_class_list"]
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["class-list"]["stats"] if info != "NotFound" else info
