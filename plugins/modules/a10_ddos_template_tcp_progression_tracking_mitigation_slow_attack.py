...
DOCUMENTATION = r'''
module: a10_ddos_template_tcp_progression_tracking_mitigation_slow_attack
description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/template/tcp/{tcp_name}/progression-tracking/mitigation/slow-attack"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["slow-attack"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["slow-attack"].get(k) != v:
            change_results["changed"] = True
            config_changes["slow-attack"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("slow-attack", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["slow-attack"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["slow-attack-list"] if info != "NotFound" else info
