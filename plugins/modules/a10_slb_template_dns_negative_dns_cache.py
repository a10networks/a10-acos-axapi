...
DOCUMENTATION = r'''
module: a10_slb_template_dns_negative_dns_cache
description:
    - setting the nagative dns cache
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/slb/template/dns/{dns_name}/negative-dns-cache"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["negative-dns-cache"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["negative-dns-cache"].get(k) != v:
            change_results["changed"] = True
            config_changes["negative-dns-cache"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("negative-dns-cache", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["negative-dns-cache"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["negative-dns-cache-list"] if info != "NotFound" else info
