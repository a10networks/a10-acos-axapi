...
short_description: Configures A10 ip.extcommunity.list.standard-num
...
def get_argspec():
    ...
    rv.update(dict(
        rules_list=dict(type='list', std_list_action=dict(type='str', choices=['deny', 'permit']), std_list_value=dict(type='str', )),
        uuid=dict(type='str', ),
        std_list_num=dict(type='int', required=True, )
    ))
        ...
        for k, v in payload["standard-num"].items():
            if isinstance(v, str):
               ...
               break
                ...
                if existing_config["standard-num"][k] != v:
                    if result["changed"] != True:
                        result["changed"] = True
                    existing_config["standard-num"][k] = v
