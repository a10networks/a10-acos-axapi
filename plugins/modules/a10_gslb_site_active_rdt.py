...
AVAILABLE_PROPERTIES = ["aging_time", "bind_geoloc", "ignore_count", "ipv6_mask", "limit", "mask", "overlap", "range_factor", "smooth_factor", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'aging_time': {'type': 'int', },
        'smooth_factor': {'type': 'int', },
        'range_factor': {'type': 'int', },
        'limit': {'type': 'int', },
        'mask': {'type': 'str', },
        'ipv6_mask': {'type': 'int', },
        'ignore_count': {'type': 'int', },
        'bind_geoloc': {'type': 'bool', },
        'overlap': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["active-rdt"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["active-rdt"].get(k) != v:
            change_results["changed"] = True
            config_changes["active-rdt"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("active-rdt", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["active-rdt"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["active-rdt-list"] if info != "NotFound" else info
