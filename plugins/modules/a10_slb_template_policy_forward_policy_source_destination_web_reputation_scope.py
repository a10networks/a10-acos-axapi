...
AVAILABLE_PROPERTIES = ["action", "dual_stack_action", "priority", "stats", "ntype", "uuid", "web_reputation_scope", ]
...
def new_url(module):
    ...
    url_base = "/axapi/v3/slb/template/policy/{policy_name}/forward-policy/source/{name}/destination/web-reputation-scope"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["web-reputation-scope"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["web-reputation-scope"].get(k) != v:
            change_results["changed"] = True
            config_changes["web-reputation-scope"][k] = v
