...
options:
    ...
    lb_method:
        ...
        - "'dst-ip-hash'= Load-balancing based on only Dst IP and Port hash; 'dst-ip-only-
          ...
          Weighted round robin on server level; 'weighted-rr-interleave'= Weighted round
          robin interleave on server level; 'service-weighted-rr'= Weighted round robin
          on service port level; 'round-robin'= Round robin on server level; 'round-
          robin-strict'= Strict mode round robin on server level; 'odd-even-hash'=
...
def get_argspec():
    ...
    rv.update({
        ...
        'lb_method': {
            'type': 'str',
            'choices': ['dst-ip-hash', 'dst-ip-only-hash', 'fastest-response', 'least-request', 'src-ip-hash', 'src-ip-only-hash', 'weighted-rr', 'weighted-rr-interleave', 'service-weighted-rr', 'round-robin', 'round-robin-strict', 'odd-even-hash']
            },
        'lrprr_method': {
            'type': 'str',
