...
DOCUMENTATION = r'''
module: a10_ddos_dst_entry_src_dst_pair_policy
description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/src-dst-pair-policy/{src_based_policy_name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/src-dst-pair-policy"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["src-dst-pair-policy"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["src-dst-pair-policy"].get(k) != v:
            change_results["changed"] = True
            config_changes["src-dst-pair-policy"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("src-dst-pair-policy", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["src-dst-pair-policy"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["src-dst-pair-policy-list"] if info != "NotFound" else info
