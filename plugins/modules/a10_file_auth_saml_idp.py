...
AVAILABLE_PROPERTIES = ["action", "dst_file", "file", "file_handle", "oper", "uuid", "verify_xml_signature", ]
...
def get_argspec():
    ...
    rv.update({'file_path': {'type': 'str', },'file': {'type': 'str', },
        ...
        'verify_xml_signature': {'type': 'bool', },
        'dst_file': {'type': 'str', },
        'uuid': {'type': 'str', },
        'oper': {'type': 'dict', 'file_list': {'type': 'list', 'file': {'type': 'str', }}}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["auth-saml-idp"].items():
        if k not in file_check:
            continue
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["auth-saml-idp"].get(k) != v:
            change_results["changed"] = True
            config_changes["auth-saml-idp"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("auth-saml-idp", module.params, AVAILABLE_PROPERTIES)
...
def run_command(module):
    ...
    try:
        ...
        existing_config, file_exists = api_client.get_file(
            module.client, "auth-saml-idp", file_url,
            module.params['file'])
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["auth-saml-idp"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["auth-saml-idp-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module),
                                                      params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["auth-saml-idp"]["oper"] if info != "NotFound" else info
