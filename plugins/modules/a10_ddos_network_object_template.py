...
DOCUMENTATION = r'''
module: a10_ddos_network_object_template
description:
...
AVAILABLE_PROPERTIES = ["anomaly_detection_trigger", "flooding_multiplier", "histogram_mode", "host_anomaly_threshold", "indicators_to_monitor", "name", "network_object_anomaly_threshold", "operational_mode", "service_break_down_threshold_local", "service_discovery", "sport_anomaly_threshold", "threshold_sensitivity", "user_tag", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object-template/{name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/network-object-template"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["network-object-template"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["network-object-template"].get(k) != v:
            change_results["changed"] = True
            config_changes["network-object-template"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("network-object-template", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["network-object-template"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["network-object-template-list"] if info != "NotFound" else info
