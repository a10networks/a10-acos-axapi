...
def get_argspec():
    ...
    rv.update({'geo_name': {'type': 'str', 'required': True, },
        'alias': {'type': 'list', 'alias': {'type': 'str', }},
        'action': {'type': 'bool', },
        'action_type': {'type': 'str', 'choices': ['allow', 'drop', 'forward', 'ignore', 'reject']},
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/geo-location/{geo_name}"

    f_dict = {}
    if '/' in str(module.params["geo_name"]):
        f_dict["geo_name"] = module.params["geo_name"].replace("/","%2F")
    else:
        f_dict["geo_name"] = module.params["geo_name"]
