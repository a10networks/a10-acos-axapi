...
def get_argspec():
    ...
    rv.update({'attr_num': {'type': 'int', 'required': True, },
        'attribute_name': {'type': 'str', },
        'any': {'type': 'bool', },
        'attr_type': {'type': 'bool', },
        'string_type': {'type': 'bool', },
        'integer_type': {'type': 'bool', },
        'ip_type': {'type': 'bool', },
        'number_type': {'type': 'bool', },
        'attr_str': {'type': 'str', 'choices': ['match', 'sub-string']},
        'attr_str_val': {'type': 'str', },
        'attr_int': {'type': 'str', 'choices': ['equal', 'not-equal', 'less-than', 'more-than', 'less-than-equal-to', 'more-than-equal-to']},
        'attr_int_val': {'type': 'int', },
        'attr_ip': {'type': 'str', 'choices': ['equal', 'not-equal']},
        'attr_ipv4': {'type': 'str', },
        'attr_number': {'type': 'str', 'choices': ['equal', 'not-equal', 'less-than', 'more-than', 'less-than-equal-to', 'more-than-equal-to']},
        'attr_number_val': {'type': 'str', },
        'A10_AX_AUTH_URI': {'type': 'bool', },
        'custom_attr_type': {'type': 'bool', },
        'custom_attr_str': {'type': 'str', 'choices': ['match', 'sub-string']},
        'a10_dynamic_defined': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    f_dict = {}
    if '/' in str(module.params["attr_num"]):
        f_dict["attr_num"] = module.params["attr_num"].replace("/","%2F")
    else:
        f_dict["attr_num"] = module.params["attr_num"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["attribute"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["attribute"].get(k) != v:
            change_results["changed"] = True
            config_changes["attribute"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("attribute", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["attribute"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["attribute-list"] if info != "NotFound" else info
