...
def new_url(module):
    ...
    url_base = "/axapi/v3/access-list/extended"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["extended"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["extended"].get(k) != v:
            change_results["changed"] = True
            config_changes["extended"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("extended", module.params, AVAILABLE_PROPERTIES)
