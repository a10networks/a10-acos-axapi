...
def get_argspec():
    ...
    rv.update({'ip_addr': {'type': 'str', 'required': True, },
        'overlay_mac_addr': {'type': 'str', 'required': True, },
        'vni': {'type': 'int', 'required': True, },
        'remote_vtep': {'type': 'str', 'required': True, },
        'uuid': {'type': 'str', }
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    if '/' in str(module.params["overlay_mac_addr"]):
        f_dict["overlay_mac_addr"] = module.params["overlay_mac_addr"].replace("/","%2F")
    else:
        f_dict["overlay_mac_addr"] = module.params["overlay_mac_addr"]
    if '/' in str(module.params["vni"]):
        f_dict["vni"] = module.params["vni"].replace("/","%2F")
    else:
        f_dict["vni"] = module.params["vni"]
    if '/' in str(module.params["remote_vtep"]):
