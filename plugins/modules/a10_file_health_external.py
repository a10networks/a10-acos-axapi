...
DOCUMENTATION = r'''
module: a10_file_health_external
description:
...
AVAILABLE_PROPERTIES = ["action", "description", "dst_file", "file", "file_handle", "oper", "size", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'file_content': {'type': 'str', },'file': {'type': 'str', },
        ...
        'dst_file': {'type': 'str', },
        'uuid': {'type': 'str', },
        'oper': {'type': 'dict', 'file_list': {'type': 'list', 'file': {'type': 'str', }, 'desc': {'type': 'str', }, 'partition': {'type': 'str', }, 'all_partitions': {'type': 'int', }, 'content': {'type': 'int', }}}
    })
    return rv
...
def get(module):
    ...
    if file_info:
        ...
        file_list = file_info["health-external"]["oper"].get("file-list", [])
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["health-external"].items():
        if k not in file_check:
            continue
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["health-external"].get(k) != v:
            change_results["changed"] = True
            config_changes["health-external"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("health-external", module)
