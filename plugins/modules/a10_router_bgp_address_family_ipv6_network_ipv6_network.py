...
def existing_url(module):
    ...
    if '/' in str(module.params["network_ipv6"]):
        f_dict["network_ipv6"] = module.params["network_ipv6"].replace(
            "/", "%2F")
    else:
...
def present(module, result, existing_config):
    payload = utils.build_json("ipv6-network", module.params,
                               AVAILABLE_PROPERTIES)
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info[
                    "ipv6-network"] if info != "NotFound" else info
                ...
                result["acos_info"] = info[
                    "ipv6-network-list"] if info != "NotFound" else info
