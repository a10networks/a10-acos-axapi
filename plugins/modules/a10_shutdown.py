...
def get_argspec():
    ...
    rv.update({'at': {'type': 'bool', },
        'nin': {'type': 'str', },
        'cancel': {'type': 'bool', },
        'reason': {'type': 'str', },
        'time': {'type': 'str', },
        'month': {'type': 'str', 'choices': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']},
        'day_of_month': {'type': 'int', },
        'reason_2': {'type': 'str', },
        'month_2': {'type': 'str', 'choices': ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']},
        'reason_3': {'type': 'str', },
        'day_of_month_2': {'type': 'int', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["shutdown"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["shutdown"].get(k) != v:
            change_results["changed"] = True
            config_changes["shutdown"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("shutdown", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["shutdown"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["shutdown-list"] if info != "NotFound" else info
