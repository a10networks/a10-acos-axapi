...
DOCUMENTATION = r'''
module: a10_ddos_dst_zone_src_port_zone_src_port_other_stats_tcp_zone_port
description:
    - Statistics for the object zone-src-port-other
author: A10 Networks
options:
    state:
        description:
    ...
    protocol:
        ...
        type: str
        required: True
    zone_src_port_other_port_other:
        description:
        - Key to identify parent object
        type: str
        required: True
...
AVAILABLE_PROPERTIES = ["port_other", "protocol", "stats", ]
...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(protocol=dict(type='str', required=True), zone_src_port_other_port_other=dict(type='str', required=True), zone_name=dict(type='str', required=True), ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/zone/{zone_name}/src-port/zone-src-port-other/{zone_src_port_other_port_other}+{protocol}/stats?tcp-zone-port=true"
    ...
    if '/' in module.params["zone_src_port_other_port_other"]:
        f_dict["zone_src_port_other_port_other"] = module.params["zone_src_port_other_port_other"].replace("/", "%2F")
    else:
        f_dict["zone_src_port_other_port_other"] = module.params["zone_src_port_other_port_other"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["zone-src-port-other"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["zone-src-port-other"].get(k) != v:
            change_results["changed"] = True
            config_changes["zone-src-port-other"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("zone-src-port-other", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["zone-src-port-other"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["zone-src-port-other-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["zone-src-port-other"]["stats"] if info != "NotFound" else info
