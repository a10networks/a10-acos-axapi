...
DOCUMENTATION = r'''
module: a10_gslb_zone_service_dns_mx_record
description:
...
def get_argspec():
    ...
    rv.update({'mx_name': {'type': 'str', 'required': True, },
        'priority': {'type': 'int', },
        'ttl': {'type': 'int', },
        'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'hits']}},
        'oper': {'type': 'dict', 'last_server': {'type': 'str', }, 'mx_name': {'type': 'str', 'required': True, }},
        'stats': {'type': 'dict', 'hits': {'type': 'str', }, 'mx_name': {'type': 'str', 'required': True, }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-mx-record/{mx_name}"

    f_dict = {}
    if '/' in str(module.params["mx_name"]):
        f_dict["mx_name"] = module.params["mx_name"].replace("/","%2F")
    else:
        f_dict["mx_name"] = module.params["mx_name"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-mx-record"].items():
        ...
        if config_changes["dns-mx-record"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-mx-record"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-mx-record", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dns-mx-record"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dns-mx-record-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module),
                                                      params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["dns-mx-record"]["oper"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["dns-mx-record"]["stats"] if info != "NotFound" else info
