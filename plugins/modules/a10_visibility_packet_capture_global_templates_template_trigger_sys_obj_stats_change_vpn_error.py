...
description:
    - Configure triggers for vpn.error object
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/vpn-error"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["vpn-error"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["vpn-error"].get(k) != v:
            change_results["changed"] = True
            config_changes["vpn-error"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("vpn-error", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["vpn-error"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["vpn-error-list"] if info != "NotFound" else info
