...
AVAILABLE_PROPERTIES = ["admin_ip", "as_backup", "as_replace", "disable", "no_resp", "static", "stats", "svrname", "ttl", "uuid", "weight", ]
...
def get_argspec():
    ...
    rv.update({'svrname': {'type': 'str', 'required': True, },
        'no_resp': {'type': 'bool', },
        'as_backup': {'type': 'bool', },
        'weight': {'type': 'int', },
        'ttl': {'type': 'int', },
        ...
        'admin_ip': {'type': 'int', },
        'uuid': {'type': 'str', },
        'stats': {'type': 'dict', 'hits': {'type': 'str', }, 'svrname': {'type': 'str', 'required': True, }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-a-record/dns-a-record-srv/{svrname}"

    f_dict = {}
    if '/' in str(module.params["svrname"]):
        f_dict["svrname"] = module.params["svrname"].replace("/","%2F")
    else:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-a-record-srv"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["dns-a-record-srv"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-a-record-srv"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-a-record-srv", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dns-a-record-srv"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dns-a-record-srv-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["dns-a-record-srv"]["stats"] if info != "NotFound" else info
