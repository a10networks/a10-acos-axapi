...
AVAILABLE_PROPERTIES = [
    "append_mapping_info", "counter_polling_interval", "default_counter_polling_mtu", "local_collection", "local_t1_polling_interval", "local_t2_polling_interval", "management_link_utilization", "management_link_utilization_percentage", "max_header", "packet_sampling_rate", "port_range_end", "port_range_start", "randomize_source_port",
    "source_ip_use_mgmt", "uuid",
    ]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["setting"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["setting"].get(k) != v:
            change_results["changed"] = True
            config_changes["setting"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("setting", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["setting"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["setting-list"] if info != "NotFound" else info
