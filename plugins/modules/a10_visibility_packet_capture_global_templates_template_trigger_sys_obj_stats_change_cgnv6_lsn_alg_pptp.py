...
DOCUMENTATION = r'''
module: a10_visibility_packet_capture_global_templates_template_trigger_sys_obj_stats_change_cgnv6_lsn_alg_pptp
description:
    - Configure triggers for cgnv6.lsn.alg.pptp object
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/cgnv6-lsn-alg-pptp"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["cgnv6-lsn-alg-pptp"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["cgnv6-lsn-alg-pptp"].get(k) != v:
            change_results["changed"] = True
            config_changes["cgnv6-lsn-alg-pptp"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("cgnv6-lsn-alg-pptp", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["cgnv6-lsn-alg-pptp"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["cgnv6-lsn-alg-pptp-list"] if info != "NotFound" else info
