...
DOCUMENTATION = r'''
module: a10_ddos_dst_entry_src_port_range_stats_tcp_port
description:
    - Statistics for the object src-port-range
author: A10 Networks
options:
    state:
        description:
    ...
    src_port_range_end:
        ...
        type: str
        required: True
    src_port_range_src_port_range_start:
        description:
        - Key to identify parent object
        type: str
        required: True
...
AVAILABLE_PROPERTIES = ["protocol", "src_port_range_end", "src_port_range_start", "stats", ]
...
def get_argspec():
    ...
    # Parent keys
    rv.update(dict(protocol=dict(type='str', required=True), src_port_range_end=dict(type='str', required=True), src_port_range_src_port_range_start=dict(type='str', required=True), entry_dst_entry_name=dict(type='str', required=True), ))
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/src-port-range/{src_port_range_src_port_range_start}+{src_port_range_end}+{protocol}/stats?tcp-port=true"
    ...
    if '/' in module.params["src_port_range_end"]:
        f_dict["src_port_range_end"] = module.params["src_port_range_end"].replace("/", "%2F")
    else:
        f_dict["src_port_range_end"] = module.params["src_port_range_end"]
    if '/' in module.params["src_port_range_src_port_range_start"]:
        f_dict["src_port_range_src_port_range_start"] = module.params["src_port_range_src_port_range_start"].replace("/", "%2F")
    else:
        f_dict["src_port_range_src_port_range_start"] = module.params["src_port_range_src_port_range_start"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["src-port-range"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["src-port-range"].get(k) != v:
            change_results["changed"] = True
            config_changes["src-port-range"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("src-port-range", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["src-port-range"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["src-port-range-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module), params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["src-port-range"]["stats"] if info != "NotFound" else info
