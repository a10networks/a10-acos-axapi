...
DOCUMENTATION = r'''
module: a10_ddos_dst_entry_capture_config
description:
...
AVAILABLE_PROPERTIES = ["mode", "name", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, }, 'mode': {'type': 'str', 'choices': ['drop', 'forward', 'all']}, 'uuid': {'type': 'str', }})
    # Parent keys
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/capture-config/{name}"
...
def new_url(module):
    ...
    url_base = "/axapi/v3/ddos/dst/entry/{entry_dst_entry_name}/capture-config"
...
def present(module, result, existing_config):
    payload = utils.build_json("capture-config", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single" or module.params.get("get_type") is None:
                ...
                result["acos_info"] = info["capture-config"] if info != "NotFound" else info
