...
def get_argspec():
    ...
    rv.update({'icmp_timeout': {'type': 'dict', 'icmp_timeout': {'type': 'str', 'choices': ['age', 'fast']}, 'icmp_timeout_val': {'type': 'int', }},
        'tcp_timeout': {'type': 'int', },
        'udp_timeout': {'type': 'int', },
        'ignore_tcp_msl': {'type': 'bool', },
        'uuid': {'type': 'str', },
        'service_timeout_list': {'type': 'list', 'service_type': {'type': 'str', 'required': True, 'choices': ['tcp', 'udp']}, 'port': {'type': 'int', 'required': True, }, 'timeout_type': {'type': 'str', 'choices': ['age', 'fast']}, 'timeout_val': {'type': 'int', }, 'uuid': {'type': 'str', }}
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["translation"].items():
        ...
        if config_changes["translation"].get(k) != v:
            change_results["changed"] = True
            config_changes["translation"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("translation", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["translation"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["translation-list"] if info != "NotFound" else info
