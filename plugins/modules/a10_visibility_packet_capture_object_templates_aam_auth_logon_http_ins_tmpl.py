...
def get_argspec():
    ...
    rv.update({'name': {'type': 'str', 'required': True, },
        ...
        'trigger_stats_inc': {'type': 'dict', 'spn_krb_faiure': {'type': 'bool', }, 'uuid': {'type': 'str', }},
        'trigger_stats_rate': {'type': 'dict', 'threshold_exceeded_by': {'type': 'int', }, 'duration': {'type': 'int', }, 'spn_krb_faiure': {'type': 'bool', }, 'uuid': {'type': 'str', }}
    })
    return rv
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/object-templates/aam-auth-logon-http-ins-tmpl/{name}"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["aam-auth-logon-http-ins-tmpl"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["aam-auth-logon-http-ins-tmpl"].get(k) != v:
            change_results["changed"] = True
            config_changes["aam-auth-logon-http-ins-tmpl"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("aam-auth-logon-http-ins-tmpl", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["aam-auth-logon-http-ins-tmpl"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["aam-auth-logon-http-ins-tmpl-list"] if info != "NotFound" else info
