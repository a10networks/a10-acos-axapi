...
DOCUMENTATION = r'''
module: a10_ddos_zone_template_sip_malformed_sip
description:
...
AVAILABLE_PROPERTIES = ["malformed_sip_action", "malformed_sip_action_list_name", "malformed_sip_call_id_max_length", "malformed_sip_check", "malformed_sip_max_header_name_length", "malformed_sip_max_header_value_length", "malformed_sip_max_line_size", "malformed_sip_max_uri_length", "malformed_sip_sdp_max_length", "uuid", ]
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/ddos/zone-template/sip/{sip_tmpl_name}/malformed-sip"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["malformed-sip"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["malformed-sip"].get(k) != v:
            change_results["changed"] = True
            config_changes["malformed-sip"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("malformed-sip", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["malformed-sip"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["malformed-sip-list"] if info != "NotFound" else info
