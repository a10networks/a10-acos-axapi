...
def get_argspec():
    ...
    rv.update({'record_name': {'type': 'str', 'required': True, },
        'txt_data': {'type': 'str', },
        'ttl': {'type': 'int', },
        'uuid': {'type': 'str', },
        'sampling_enable': {'type': 'list', 'counters1': {'type': 'str', 'choices': ['all', 'hits']}},
        'stats': {'type': 'dict', 'hits': {'type': 'str', }, 'record_name': {'type': 'str', 'required': True, }}
    })
    # Parent keys
    rv.update(dict(
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/gslb/zone/{zone_name}/service/{service_port}+{service_name}/dns-txt-record/{record_name}"

    f_dict = {}
    if '/' in str(module.params["record_name"]):
        f_dict["record_name"] = module.params["record_name"].replace("/","%2F")
    else:
        f_dict["record_name"] = module.params["record_name"]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["dns-txt-record"].items():
        ...
        if config_changes["dns-txt-record"].get(k) != v:
            change_results["changed"] = True
            config_changes["dns-txt-record"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("dns-txt-record", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["dns-txt-record"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["dns-txt-record-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "stats":
                get_type_result = api_client.get_stats(module.client, existing_url(module),
                                                       params=module.params)
                result["axapi_calls"].append(get_type_result)
                info = get_type_result["response_body"]
                result["acos_info"] = info["dns-txt-record"]["stats"] if info != "NotFound" else info
