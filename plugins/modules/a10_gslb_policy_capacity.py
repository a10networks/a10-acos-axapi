...
short_description: Configures A10 gslb.policy.capacity
...
options:
    ...
    policy_name:
        description:
        - Key to identify parent object    threshold:
        description:
    ...
    if existing_config:
        for k, v in payload["capacity"].items():
            if isinstance(v, str):
                ...
                if existing_config["capacity"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["capacity"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("capacity", module)
