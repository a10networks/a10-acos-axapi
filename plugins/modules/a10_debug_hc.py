...
def get_argspec():
    ...
    rv.update({'metrics': {'type': 'bool', },
        'object_uuid': {'type': 'str', },
        'uri': {'type': 'str', },
        'per_request': {'type': 'bool', },
        'app_svc_id': {'type': 'str', },
        'anomaly': {'type': 'bool', },
        'registration': {'type': 'bool', },
        'error': {'type': 'bool', },
        'per_connection': {'type': 'bool', },
        'uuid': {'type': 'str', }
    })
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["hc"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["hc"].get(k) != v:
            change_results["changed"] = True
            config_changes["hc"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("hc", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["hc"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["hc-list"] if info != "NotFound" else info
