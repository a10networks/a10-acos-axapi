...
AVAILABLE_PROPERTIES = ["enable", "local_ip", "oper", "peer_ip_cfg", "uuid", ]
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["sync"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["sync"].get(k) != v:
            change_results["changed"] = True
            config_changes["sync"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("sync", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["sync"] if info != "NotFound" else info
