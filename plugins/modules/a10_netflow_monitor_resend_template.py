...
short_description: Configures A10 netflow.monitor.resend-template
...
options:
    ...
    monitor_name:
        description:
        - Key to identify parent object    records:
        description:
    ...
    if existing_config:
        for k, v in payload["resend-template"].items():
            if isinstance(v, str):
                ...
                if existing_config["resend-template"][k] != v:
                    if result["changed"] is not True:
                        result["changed"] = True
                    existing_config["resend-template"][k] = v
...
def present(module, result, existing_config):
    payload = build_json("resend-template", module)
