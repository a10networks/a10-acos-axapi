...
DOCUMENTATION = r'''
module: a10_sys_ut_state_next_state_case
description:
    - Case number
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/sys-ut/state/{state_name}/next-state/{name}/case/{case_number}"

    f_dict = {}
    if '/' in str(module.params["case_number"]):
        f_dict["case_number"] = module.params["case_number"].replace("/","%2F")
    else:
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["case"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["case"].get(k) != v:
            change_results["changed"] = True
            config_changes["case"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("case", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["case"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["case-list"] if info != "NotFound" else info
