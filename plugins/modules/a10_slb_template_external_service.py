...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["external-service"].items():
        ...
        if config_changes["external-service"].get(k) != v:
            change_results["changed"] = True
            config_changes["external-service"][k] = v
...
def present(module, result, existing_config):
    payload = utils.build_json("external-service", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["external-service"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["external-service-list"] if info != "NotFound" else info
