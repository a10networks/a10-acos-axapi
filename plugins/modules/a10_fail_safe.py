...
AVAILABLE_PROPERTIES = ["config", "dataplane_recovery_timeout", "disable_failsafe", "fpga_buff_recovery_threshold", "fpga_monitor_enable", "fpga_monitor_forced_reboot", "fpga_monitor_interval", "fpga_monitor_threshold", "hw_error_monitor", "hw_error_recovery_timeout", "hw_ssl_timeout_monitor", "kill", "log", "oper", "session_mem_recovery_threshold", "sw_error_monitor_enable", "sw_error_recovery_timeout", "total_memory_size_check", "uuid", ]
...
def get_argspec():
    ...
    rv.update({'fpga_buff_recovery_threshold': {'type': 'int', },
        ...
        'dataplane_recovery_timeout': {'type': 'int', },
        'log': {'type': 'bool', },
        'kill': {'type': 'bool', },
        'uuid': {'type': 'str', },
...
def present(module, result, existing_config):
    payload = utils.build_json("fail-safe", module.params, AVAILABLE_PROPERTIES)
    ...
    try:
        ...
        if state == 'noop':
            if module.params.get("get_type") == "single":
                ...
                result["acos_info"] = info["fail-safe"] if info != "NotFound" else info
            elif module.params.get("get_type") == "list":
                get_list_result = api_client.get_list(module.client, existing_url(module))
                result["axapi_calls"].append(get_list_result)

                info = get_list_result["response_body"]
                result["acos_info"] = info["fail-safe-list"] if info != "NotFound" else info
            elif module.params.get("get_type") == "oper":
                get_oper_result = api_client.get_oper(module.client, existing_url(module),
                                                      params=module.params)
                result["axapi_calls"].append(get_oper_result)
                info = get_oper_result["response_body"]
                result["acos_info"] = info["fail-safe"]["oper"] if info != "NotFound" else info
