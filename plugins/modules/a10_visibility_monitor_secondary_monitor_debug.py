...
DOCUMENTATION = r'''
module: a10_visibility_monitor_secondary_monitor_debug
description:
...
def get_argspec():
    ...
    rv.update({'debug_ip_addr': {'type': 'str', 'required': True, }, 'debug_port': {'type': 'int', 'required': True, }, 'debug_protocol': {'type': 'str', 'required': True, 'choices': ['TCP', 'UDP', 'ICMP']}, 'uuid': {'type': 'str', }})
    return rv
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["debug"].items():
        ...
        if config_changes["debug"].get(k) != v:
            change_results["changed"] = True
            config_changes["debug"][k] = v
