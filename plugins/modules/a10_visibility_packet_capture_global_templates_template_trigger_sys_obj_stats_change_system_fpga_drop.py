...
description:
    - Configure triggers for system.fpga-drop object
author: A10 Networks
options:
    state:
        description:
...
def existing_url(module):
    ...
    url_base = "/axapi/v3/visibility/packet-capture/global-templates/template/{template_name}/trigger-sys-obj-stats-change/system-fpga-drop"
...
def report_changes(module, result, existing_config, payload):
    ...
    for k, v in payload["system-fpga-drop"].items():
        v = 1 if str(v).lower() == "true" else v
        v = 0 if str(v).lower() == "false" else v

        if config_changes["system-fpga-drop"].get(k) != v:
            change_results["changed"] = True
            config_changes["system-fpga-drop"][k] = v
